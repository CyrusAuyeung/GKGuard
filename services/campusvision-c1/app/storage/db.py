...
def purge_live_videos(camera_id: str, before_created_at: str) -> dict[str, Any]:
    with get_conn(write=True) as conn:
        ...
        touched_person_ids = {row["person_id"] for row in event_rows if row["person_id"]}
        ...
        face_person_rows = conn.execute(
            f"""
            SELECT DISTINCT pf.person_id
            FROM person_faces pf
            JOIN face_records fr ON fr.face_id = pf.face_id
            WHERE fr.video_id IN ({placeholders})
            """,
            video_ids,
        ).fetchall()
        touched_person_ids.update(row["person_id"] for row in face_person_rows if row["person_id"])
    ...
    return {
        ...
        "touched_person_ids": sorted(touched_person_ids),
        "videos": videos,
    }
...
def delete_person(person_id: str) -> dict[str, int]:
    ...
    with get_conn(write=True) as conn:
        ...
        conn.execute(
            """
            UPDATE person_observations
            SET person_id = NULL, updated_at = ?
            WHERE person_id = ?
            """,
            (ts, person_id),
        )
        updated_observations = conn.execute("SELECT changes()").fetchone()[0]
        conn.execute(
            """
            UPDATE events SET
                person_id = NULL,
        ...
        updated_events = conn.execute("SELECT changes()").fetchone()[0]
        conn.execute("DELETE FROM persons WHERE person_id = ?", (person_id,))
        deleted_persons = conn.execute("SELECT changes()").fetchone()[0]
    return {
