...
当前 `v0.3.0` 集成状态下，CampusVision C1 还提供人物事件、事件观测、人体图、上装颜色、眼镜状态、外观倾向和人物特征检索接口，供 GKGuard C2 在工作台导航中展示“人脸以图搜人”和“人物特征搜索”两个入口。GKGuard C2 会把 CampusVision C1 返回的人物、事件、观测、关键帧、人脸裁剪图、人体裁剪图和轨迹字段归一化为结果记录、证据详情、候选人物抽屉、事件详情抽屉和路线图。
...
In the current `v0.3.0` integration state, CampusVision C1 also exposes person events, event observations, body crops, upper color, glasses status, appearance presentation, and person-attribute search endpoints for GKGuard C2's `人脸以图搜人` and `人物特征搜索` workbench entries. GKGuard C2 normalizes returned persons, events, observations, keyframes, face crops, body crops, and trajectory fields into result records, evidence detail, candidate-person drawers, event-detail drawers, and the route map.
