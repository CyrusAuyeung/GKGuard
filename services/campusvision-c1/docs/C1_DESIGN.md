...
```
`INSIGHTFACE_DET_SIZE` 默认 `1280`，用于提高多人合照、教室和乐队场景中的查询图人脸召回；修改后需要重启实际监听端口的 uvicorn worker。
...
本地开发如果使用旧环境或错误 worker，可能出现 `/health` 仍显示旧引擎或多人图只检测到一张脸的情况。需要确认实际监听端口的 uvicorn worker 环境变量。
...
GKGuard C2 当前通过自身 `/c1/...` 代理和适配器转发这些能力，避免前端直接依赖 CampusVision C1 内部结构。查询图片有多张有效候选人脸时，GKGuard C2 先调用 `/api/v1/search/query-faces` 获取 `query_faces[].bbox` 和检测置信度，隐藏低置信误检框后，再把用户选择的人脸序号作为 `query_face_index` 传给 `/api/v1/search/person-by-image`。检索结果中的 `matches[].bbox` 用于在 GKGuard C2 关键帧中标出目标人脸；GKGuard C2 适配器兼容像素坐标、归一化坐标和百分比字段，并按实际图片内容区域渲染目标框。若检索返回无匹配结果，GKGuard C2 保持上传页并提示无匹配，不进入本地模拟结果。
...
INSIGHTFACE_DET_SIZE=1280
```
`INSIGHTFACE_DET_SIZE` defaults to `1280` and improves query-face recall for group photos, classrooms, and band-room scenes. Restart the actual uvicorn worker after changing it.
...
In local development, an old environment or stale worker can make `/health` still show an old engine or make multi-face images detect only one face. Confirm the environment variables of the actual uvicorn worker that owns the listening port.
...
GKGuard C2 currently forwards these capabilities through its own `/c1/...` proxy and adapter so the frontend does not depend on CampusVision C1 internals. For query images with multiple effective candidate faces, GKGuard C2 calls `/api/v1/search/query-faces` first to obtain `query_faces[].bbox` and detection confidence, hides low-confidence false-positive boxes, then passes the selected face index as `query_face_index` to `/api/v1/search/person-by-image`. `matches[].bbox` in search results is used to mark the target face on GKGuard C2 keyframes; the GKGuard C2 adapter accepts pixel coordinates, normalized coordinates, and percentage fields, and renders the target box against the actual image content area. If search returns no matched records, GKGuard C2 keeps the upload screen active with a no-match warning instead of entering local mock results.
