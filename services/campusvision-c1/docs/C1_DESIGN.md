...
GKGuard C2 当前通过自身 `/c1/...` 代理和适配器转发这些能力，避免前端直接依赖 CampusVision C1 内部结构。查询图片有多张人脸时，GKGuard C2 先调用 `/api/v1/search/query-faces` 获取 `query_faces[].bbox` 和检测置信度，再把用户选择的人脸序号作为 `query_face_index` 传给 `/api/v1/search/person-by-image`。检索结果中的 `matches[].bbox` 用于在 GKGuard C2 关键帧中标出目标人脸；GKGuard C2 适配器兼容像素坐标、归一化坐标和百分比字段。
...
GKGuard C2 currently forwards these capabilities through its own `/c1/...` proxy and adapter so the frontend does not depend on CampusVision C1 internals. For query images with multiple faces, GKGuard C2 calls `/api/v1/search/query-faces` first to obtain `query_faces[].bbox` and detection confidence, then passes the selected face index as `query_face_index` to `/api/v1/search/person-by-image`. `matches[].bbox` in search results is used to mark the target face on GKGuard C2 keyframes; the GKGuard C2 adapter accepts pixel coordinates, normalized coordinates, and percentage fields.
