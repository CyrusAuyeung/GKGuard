...
GKGuard C2 当前通过自身 `/c1/...` 代理和适配器转发这些能力，避免前端直接依赖 CampusVision C1 内部结构。查询图片有多张人脸时，GKGuard C2 先调用 `/api/v1/search/query-faces` 获取 `query_faces[].bbox` 和检测置信度，再把用户选择的人脸序号作为 `query_face_index` 传给 `/api/v1/search/person-by-image`。检索结果中的 `matches[].bbox` 用于在 GKGuard C2 关键帧中标出目标人脸。
