...
- 合并前还应检查 PR 正文 reaction、Issue 评论、审查线程和审查状态。👀 / 👍 这类状态表情只以 PR 正文 reaction 为准，不以 review 后的评论 reaction 为准。没有任何审查信号时应继续等待或轮询，不得把“未发现评论”当作可合并状态；PR 正文 👀 或等价信号表示仍在 review，不得合并；PR 正文 👍、明确 approval 或明确无阻断结论表示当前 review 已完成且无阻断问题；Issue 评论、review 评论或未解决线程表示存在反馈，必须处理后重新等待审查信号。
