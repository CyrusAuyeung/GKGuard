...
GKGuard 当前是课程/项目演示仓库，治理目标是保持 GKGuard C2 工作台边界清晰、Release 可复现、文档可维护、敏感数据不进入仓库。

## 角色与职责

- GKGuard C2 维护者：负责 `backend/`、`desktop/`、`backend/app/static/`、Release workflow 和仓库文档。
- CampusVision C1 服务维护者：负责 `services/campusvision-c1/` 的算法服务、视频索引、人物库、以图搜人和 CampusVision C1 运行数据。
...
1. 先保护敏感数据，再追求演示便利。
2. GKGuard C2 前端只访问 GKGuard C2 后端；跨服务访问通过 GKGuard C2 adapter 或明确合同。
...
GKGuard is currently a coursework/project demo repository. Governance focuses on keeping the GKGuard C2 workbench boundary clear, Releases reproducible, documentation maintainable, and sensitive data out of the repository.

## Roles And Responsibilities

- GKGuard C2 maintainers: own `backend/`, `desktop/`, `backend/app/static/`, the Release workflow, and repository documentation.
- CampusVision C1 service maintainers: own `services/campusvision-c1/`, algorithm service, video indexing, person index, image search, and CampusVision C1 runtime data.
...
2. The GKGuard C2 frontend calls only the GKGuard C2 backend; cross-service access goes through a GKGuard C2 adapter or explicit contract.
