...
GKGuard 当前采用 **source-available / 保留所有权利** 的发布方式。仓库对协作、课程评审、演示和维护开放查看，但这不等同于开源授权。

## 授权状态

- 本仓库没有采用 MIT、Apache-2.0、GPL 等开源许可证。
- 除非仓库所有者另行书面授权，否则不得复制、再分发、转授权、商用、作为竞品服务托管，或将本代码用于其他产品。
- 允许在 GKGuard 项目协作、展示、评审和团队维护范围内查看、运行和讨论代码。
...
- Windows 安装包用于 C2 桌面演示和项目运行检查。
- 软件可离线使用 mock fallback 演示界面流程。
- 真实 C1 检索依赖校园网、VPN、SSH 隧道或服务器直连策略。
- GKGuard 不保存服务器密码；安装版只在软件内 C1 连接窗口中接收本次 SSH 连接所需密码，手动排障时也只应输入在系统 PowerShell/SSH 窗口中。
...
GKGuard is currently released as **source-available / all rights reserved**. The repository is visible for collaboration, coursework review, demonstrations, and maintenance, but this is not an open-source license grant.
...
- You may view, run, and discuss the code within the GKGuard project collaboration, coursework, review, and team maintenance context.
...
- The Windows installer is intended for C2 desktop demonstrations and project runtime checks.
...
- GKGuard does not store server passwords. The packaged app accepts the password only in its embedded C1 connection window for the current SSH session; during manual troubleshooting, passwords should only be entered in the system PowerShell/SSH window.
