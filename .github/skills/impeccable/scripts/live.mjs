...
The agent should then:
  ...
  const injectOut = runScript('live-inject.mjs', ['--port', String(serverInfo.port), '--token', String(serverInfo.token || '')], { cwd: activeCwd });
...
function runScript(name, args, options = {}) {
  ...
  try {
    return execFileSync(process.execPath, [scriptPath, ...args.map(String)], {
      encoding: 'utf-8',
