...
本仓库覆盖 GKGuard C2 的完整工程形态：本地模拟演示、FastAPI 服务接口、静态前端、Electron 桌面运行环境、CampusVision C1 真实检索接入、查询图目标人脸选择、人物特征搜索、事件关键帧展示、候选人物按身份复核、人物路线联动、Windows/macOS/Linux 桌面端发布、安全加固、协作流程管理和配套文档体系。当前最新版本是 `v0.3.2`。
...
- **人脸以图搜人前端流程**：静态前端覆盖 `人脸以图搜人`、`检索结果`、`人物路线图` 三个核心工作区；支持上传目标图片、查询图人脸检测、单人自动检索、多人目标选择、放大弹窗选人、低置信候选提示、重新上传、结果列表、关键帧预览、结果记录切换关键帧预加载、候选人物抽屉、路线查看和导出入口。
- **人物特征搜索前端流程**：当前版本包含人物特征搜索入口，可按上装颜色、眼镜状态、外观倾向、摄像头和可选时间范围查询 CampusVision C1 事件；时间输入默认显示用户当天 `00:00` 至 `23:59`，但只有操作员编辑时间字段后才提交 `time_range`，避免默认显示值隐式过滤历史事件。结果复用检索记录、关键帧、目标人脸框、相似度、路线联动、候选人物抽屉和导出展示模型。`v0.3.2` 继承 `v0.3.0` 的固定工作区导航、证据优先详情布局和响应式抽屉结构，并在 `v0.3.1` 基础上修复人物特征搜索的记录头像同步、目标框标注和候选人物按身份聚合筛选问题。
...
- 当前稳定基线为 `v0.3.2`：主流程已覆盖本地 mock 演示、CampusVision C1 真实以图搜人和 CampusVision C1 人物特征事件搜索，Windows/macOS/Linux 桌面端发布链路和本地开发后端都可进入 `/demo`。
...
- GKGuard C2 前端已完成人脸以图搜人、人物特征搜索、结果页和路线页工作流：上传页处理单人/多人查询图和异常状态，特征检索页按外观条件查询 CampusVision C1 事件，时间输入默认显示用户当天但仅在用户编辑后作为筛选条件提交，结果页展示人物照片、命中记录、关键帧、目标框、属性摘要和相似度标注，并通过候选人物抽屉按身份筛选记录，路线页展示轨迹点、时间线和当前轨迹摘要。
- 人脸选择与结果展示已针对真实照片场景做稳定性处理：多人图必须确认目标人物，低置信候选保留可见提示，无匹配/超时/检测失败会回到可继续操作的上传状态，结果页人物照片和关键帧标注按实际目标框与图片内容区域定位；人物特征搜索切换左侧记录时，左侧人物照片会同步为当前记录对应人脸，关键帧也会显示目标人脸框；中等宽度窗口下人物照片不会覆盖数据来源和命中记录信息；切换左侧命中记录时保留当前关键帧，预加载并解码目标关键帧后再替换，失败预加载可重试，快速切回当前记录时旧加载任务不会覆盖当前详情。
...
- CampusVision C1 重建索引流程已在提交前暂存帧文件，并在提交失败时恢复既有索引、帧目录、人物索引和相关 appearance sessions；数据库写入路径使用写锁隔离，读取接口不因重建索引全局串行化。
...
- Pull Request 合并前需要 `.github/workflows/ci.yml` 中的 `Verify` 检查通过，完成审查，解决所有对话，并检查 PR 正文 reaction、Issue 评论、审查线程和审查状态。👀 / 👍 这类审查状态 reaction 只存在且只读取 PR 正文下方的 reaction，不存在于 review 后评论，也不以 review/comment 下方 reaction 作为审查状态。没有任何审查信号时应继续等待或轮询；只有在没有 PR 正文审查信号且需要主动启动审查时，才可评论 `@codex review`；PR 正文已有 👀 时表示审查已经在进行中，不要再次手动触发 review，只等待结果。PR 正文 👀 表示仍在 review，不要合并；PR 正文 👍、approval 或明确无阻断结论表示当前 review 已完成；Issue 评论、review 评论或未解决线程表示需要处理反馈。仓库使用 squash merge，合并后删除源分支。
...
- `GET /c1/events`、`GET /c1/persons/{person_id}/events`、`GET /c1/events/{event_id}/observations`：代理 CampusVision C1 人物事件、事件详情和事件观测。
- `POST /c1/query/face-image`：调用 CampusVision C1 查询图候选接口，返回可用于跨人物库确认的候选人和事件摘要。
- `POST /c1/query/person-attributes`：按上装颜色、眼镜状态、外观倾向、摄像头和时间范围查询 CampusVision C1 事件，并归一化为结果页记录。
- `GET /c1/media/{media_path}`：代理 CampusVision C1 关键帧、人脸裁剪图、人体裁剪图和事件代表图。
...
```powershell
npm version 0.3.x --no-git-tag-version
git add package.json package-lock.json
git commit -m "Bump version to 0.3.x"
git push
git tag -a v0.3.x -m "GKGuard v0.3.x"
...
如果 `docs/releases/v0.3.x.md` 存在，发布工作流会优先使用这份人工维护的双语详细说明；否则会生成带中文/English 跳转、安装说明、提交和文件变更的兜底 GitHub Release 正文。
...
当前 `v0.3.2` 的基线结果：GKGuard C2 后端测试、CampusVision C1 服务端安全配置测试、前端脚本、Electron 主进程和 preload 语法检查、桌面后端入口编译、Playwright E2E、`npm audit --audit-level=low` 和 `git diff --check` 通过。Release workflow 会在 Windows、Intel macOS 和 Linux runner 上分别生成对应桌面端安装文件与 `latest*.yml` 元数据，其中 Windows 端继续提供 `GKGuard-Setup-0.3.2.exe`、`.blockmap` 和 `latest.yml`。`v0.3.2` 继承 `v0.3.0` 的工作台式信息架构：固定导航承载人脸以图搜人与人物特征搜索入口，结果页采用目标人物/记录列表与证据详情分栏，候选人物通过侧边抽屉按身份复核与筛选记录，中等宽度和移动端布局按内容密度自动收敛；本版重点修复人物特征搜索记录头像同步、目标框标注和候选人物按身份聚合筛选问题。
...
This repository covers GKGuard C2's complete engineering shape: local mock demos, FastAPI service APIs, the static frontend, the Electron desktop runtime, CampusVision C1 real-search integration, query-face target selection, person-attribute search, event keyframe presentation, candidate-person identity review, person-route synchronization, Windows/macOS/Linux desktop release flow, security hardening, collaboration workflow management, and supporting documentation. The latest version is `v0.3.2`.
...
- **Face-search frontend flow**: the static UI covers the `人脸以图搜人`, `检索结果`, and `人物路线图` work areas; it supports target-image upload, query-face detection, single-face auto-search, multi-face target selection, enlarged selection modal, low-confidence candidate hints, re-upload, result list, keyframe preview, preloaded keyframe switching between result records, candidate-person drawer, route review, and export entry points.
- **Person-attribute search frontend flow**: the current version includes a person-attribute search entry that can query CampusVision C1 events by upper-body color, glasses status, appearance presentation, camera, and an optional time range; the time inputs display the user's current day from `00:00` to `23:59` by default, but GKGuard C2 submits `time_range` only after the operator edits a time field, so the displayed defaults do not implicitly filter historical events. Results reuse the result-record, keyframe, target face box, similarity, route, candidate-person drawer, and export presentation model. `v0.3.2` inherits the fixed workbench navigation, evidence-first detail layout, and responsive drawer structure from `v0.3.0`, and fixes person-attribute record portrait synchronization, target overlays, and identity-grouped candidate filtering on top of `v0.3.1`.
...
- The current stable baseline is `v0.3.2`: the main flow supports local mock demonstration, CampusVision C1 real image search, and CampusVision C1 person-attribute event search, and the Windows/macOS/Linux desktop release flow plus the local development backend can enter `/demo`.
...
- The GKGuard C2 frontend has face search, person-attribute search, result-screen, and route-screen workflows: the upload screen handles single-face, multi-face, and error states; the attribute-search screen queries CampusVision C1 events by appearance conditions, displays today's time range by default, and submits it only after the user edits the time fields; the result screen shows the portrait, matched records, keyframes, target overlays, attribute summaries, similarity overlays, and a candidate-person drawer that filters records by identity; the route screen shows trajectory points, timeline, and current-route summary.
- Face selection and result rendering are hardened for real-photo cases: multi-face images require a confirmed target, low-confidence candidates remain visibly marked, no-match/timeout/detection-failure states return to an actionable upload screen, result portraits plus keyframe overlays are positioned from the actual target box and rendered image-content area; in person-attribute search, switching records updates the left target portrait to the selected record's face and the keyframe shows the target face box. Medium-width windows keep the portrait separate from the source and hit-count summary. When users switch records in the left list, the current keyframe stays visible until the next keyframe is preloaded and decoded, failed preloads can be retried, and stale load tasks cannot overwrite the current detail after users switch back.
- The GKGuard C2 CampusVision C1 proxy now covers `/c1/status`, `/c1/persons`, `/c1/videos`, `/c1/query-faces`, `/c1/search/person-by-image`, `/c1/events`, `/c1/persons/{person_id}/events`, `/c1/events/{event_id}/observations`, `/c1/query/face-image`, `/c1/query/person-attributes`, and extended `/c1/media/...`. It normalizes CampusVision C1 person, event, observation, feature-match, and media fields into GKGuard C2 frontend models.
- The GKGuard C2 CampusVision C1 media proxy uses a short-lived in-process cache for successfully fetched keyframes, face crops, body crops, and event representative images, scoped by the responding CampusVision C1 address, API key, candidate-URL configuration, and connection generation, with item-count, total-byte, and per-item byte limits. Fresh status confirmation or desktop SSH-tunnel reconfirmation clears media cache entries from the previous connection generation; media requests re-resolve candidates or return a retryable error if the generation changes before the GET is sent or before the response is returned, and stale media or error responses from the previous CampusVision C1 instance are not returned to the UI, written into the new cache, or allowed to overwrite the selected address. When the same media request falls back from a failed candidate to a healthy candidate, the successful candidate is preserved and cached in its resulting generation, while normal requests reuse short-lived healthy CampusVision C1 status probes.
...
- CampusVision C1 re-indexing stages frame files before commit and restores the previous index, frame directory, person-index state, and related appearance sessions if commit fails; database write paths use a write lock while read APIs avoid global serialization by the re-index lock.
...
- Before merge, Pull Requests must pass the `Verify` check from `.github/workflows/ci.yml`, complete review, resolve all conversations, and check PR-body reactions, issue comments, review threads, and review state. Review-status reactions such as 👀 / 👍 exist only below the PR body and only PR-body reactions count; reactions below review comments or other comments are not review-status signals. If no review signal is present, keep waiting or polling; only comment `@codex review` when there is no PR-body review signal and a review needs to be started actively; if the PR body already has 👀, review is already running, so do not manually retrigger review and wait for the result. PR-body 👀 means review is still in progress and the PR must not be merged; PR-body 👍, approval, or an explicit no-blocker conclusion means the current review is complete; issue comments, review comments, or unresolved threads mean feedback must be addressed. The repository uses squash merge and deletes merged head branches.
...
- `GET /c1/events`, `GET /c1/persons/{person_id}/events`, and `GET /c1/events/{event_id}/observations`: proxy CampusVision C1 person events, event detail, and event observations.
- `POST /c1/query/face-image`: calls the CampusVision C1 query-image candidate endpoint and returns candidates plus event summaries for person-index confirmation.
- `POST /c1/query/person-attributes`: queries CampusVision C1 events by upper-body color, glasses status, appearance presentation, camera, and time range, then normalizes the response into result-screen records.
- `GET /c1/media/{media_path}`: proxies CampusVision C1 keyframes, face crops, body crops, and event representative images.
...
- Offline or outside campus network: the app opens normally and can use local mock fallback for the GKGuard C2 workbench, face-image search, person-attribute search, result details, route map, and placeholder data.
...
If `docs/releases/v0.3.x.md` exists, the workflow uses that curated bilingual detailed note first; otherwise it generates a fallback Release body with Chinese/English jump links, installation notes, commits, and file changes.
...
Current `v0.3.2` baseline: GKGuard C2 backend tests, CampusVision C1 service-side security configuration tests, frontend script, Electron main-process and preload syntax checks, desktop backend entrypoint compilation, Playwright E2E, `npm audit --audit-level=low`, and `git diff --check` pass. The Release workflow generates platform-specific desktop packages and `latest*.yml` metadata on Windows, Intel macOS, and Linux runners, with Windows continuing to provide `GKGuard-Setup-0.3.2.exe`, `.blockmap`, and `latest.yml`. `v0.3.2` inherits the `v0.3.0` workbench information architecture: fixed navigation hosts image-based face search and person-attribute search, result pages split target/record context from evidence detail, candidate people move into a side drawer for identity-based review and record filtering, and medium-width/mobile layouts collapse according to content density. This patch focuses on person-attribute record portrait synchronization, target overlays, and identity-grouped candidate filtering.
