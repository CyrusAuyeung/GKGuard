...
本仓库覆盖 GKGuard C2 的完整工程形态：本地模拟演示、FastAPI 服务接口、静态前端、Electron 桌面运行环境、CampusVision C1 真实检索接入、查询图目标人脸选择、人物特征检索、事件关键帧展示、人物路线联动、Windows/macOS/Linux 桌面端发布、安全加固、协作流程管理和配套文档体系。当前最新版本是 `v0.2.2`。
...
- **人物特征检索前端流程**：当前版本包含人物特征检索入口，可按上装颜色、眼镜状态、外观倾向、摄像头和时间范围查询 CampusVision C1 事件；结果复用检索记录、关键帧、人脸框、相似度、路线联动和导出展示模型。`v0.2.2` 延续 `v0.2.0` 功能集成和 `v0.2.1` 稳定性修复，并补齐 review 后续：查询图候选接口参数位置、4xx 校验错误传递、5xx 服务错误详情脱敏、CampusVision C1 输入校验失败不触发桌面端重连、路线点唯一高亮和未映射记录的路线高亮边界。
...
- 当前稳定基线为 `v0.2.2`：主流程已覆盖本地 mock 演示、CampusVision C1 真实以图搜人和 CampusVision C1 人物特征事件检索，Windows/macOS/Linux 桌面端发布链路和本地开发后端都可进入 `/demo`。
...
当前 `v0.2.2` 的基线结果：GKGuard C2 后端测试、CampusVision C1 服务端安全配置测试、前端脚本、Electron 主进程和 preload 语法检查、桌面后端入口编译、Playwright E2E、`npm audit --audit-level=low` 和 `git diff --check` 通过；当前 Windows 本地 Python 环境缺少 `cv2`，CampusVision C1 全量视觉测试无法在本机完成，查询图预处理测试被跳过。Release workflow 会在 Windows、Intel macOS 和 Linux runner 上分别生成对应桌面端安装文件与 `latest*.yml` 元数据，其中 Windows 端继续提供 `GKGuard-Setup-0.2.2.exe`、`.blockmap` 和 `latest.yml`。CampusVision C1 服务能力和 GKGuard C2 前端入口由 `v0.2.0` 引入：CampusVision C1 包含人物事件、事件观测、外观属性分析、查询图候选和人物特征查询接口；GKGuard C2 包含人物特征检索 UI、事件结果归一化、扩展媒体代理和相关属性摘要展示。`v0.2.1` 重点修复查询图异常清理、重复视频索引、受影响人物索引与 appearance session 重建、人物特征路线顺序、路线点到结果记录的稳定映射、默认记录不在路线点内时的路线页选中状态和 API 规范示例；`v0.2.2` 在此基础上补齐 review 后续，统一 CampusVision C1 查询图候选接口 Query 参数，保留 CampusVision C1 4xx 校验错误详情，对 CampusVision C1 5xx 服务错误详情脱敏，避免图片检索和人物特征检索中的 CampusVision C1 4xx 输入校验失败触发桌面端重连，并修正重复映射路线点和未映射尾部记录的高亮边界。既有人脸检索、多人目标选择、结果页关键帧标注、记录切换预加载、路线图联动、媒体缓存隔离、受信 SSH 主机密钥自动比对和桌面端跨平台发布能力继续保留。
...
- **Person-attribute search frontend flow**: the current version includes a person-attribute search entry that can query CampusVision C1 events by upper-body color, glasses status, appearance presentation, camera, and time range; results reuse the result-record, keyframe, face-box, similarity, route, and export presentation model. `v0.2.2` continues the `v0.2.0` feature integration and the `v0.2.1` stability fixes, and completes review follow-up for query-image candidate parameter placement, 4xx validation-error propagation, 5xx service-error detail sanitization, no desktop reconnection for CampusVision C1 input-validation failures, single active route-point highlighting, and unmapped-record route-highlight boundaries.
...
- The current stable baseline is `v0.2.2`: the main flow supports local mock demonstration, CampusVision C1 real image search, and CampusVision C1 person-attribute event search, and the Windows/macOS/Linux desktop release flow plus the local development backend can enter `/demo`.
