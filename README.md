...
- GKGuard C2 前端位于 `backend/app/static/`，当前主流程为 `人脸检索`、`人脸检索结果`、`人物路线图` 三个界面；页面宽度会随桌面窗口扩展，支持 `680x640` 小窗口下无横向溢出；上传页会先检测查询图人脸，单人图自动选中并检索，多人图显示可点击人脸框并自动打开放大选择弹窗，用户可在弹窗内缩放原图并确认目标；检测置信度不等同于人物相似度，低于 `0.65` 但不低于 `0.45` 的候选会以低置信样式显示并仍可选择，低于 `0.45` 的候选不作为可选目标；若查询图人脸检测失败，不再展示伪成功结果，而是停留在上传页提示重试或检查 CampusVision C1；若 CampusVision C1 返回无匹配记录、请求超时或真实检索失败，页面会回到可继续操作的上传状态并显示中文提示，不再卡在“检索中”；结果页人物照片优先显示选中的查询人脸裁切图，桌面端记录列表恢复在左侧纵向展示并优先使用 CampusVision C1 人脸裁剪缩略图，详情区完整展示 CampusVision C1 关键帧并按实际图片内容区域定位目标人脸框，匹配相似度显示在框外，避免遮挡人脸、落在左上角或被黑边偏移，关键帧预览弹窗也保留同样标注；结果页和路线页都可一键返回重新上传；移动端上传页压缩表单和上传区高度，结果页和路线页操作按钮使用约 42px 触控高度的双列紧凑布局；移动端结果页和路线页记录列表提供横向滚动提示与右侧渐隐；路线页工具栏将 `重新上传` 固定在最左侧，并在地图前展示轨迹点、起点、终点、总时长和当前轨迹摘要，路线图点位、关联记录和时间线可联动定位当前记录，窄屏地图标签会避让边缘且图例压缩为横向条；390px 视口下模拟关键帧装饰和路线地图元素保持在容器内；检索、CampusVision C1 回退、定位、导出和更新入口使用统一状态提示，初始和切屏状态不会显示空白 toast。
- Electron 桌面壳位于 `desktop/`，GitHub Actions 可构建 Windows 安装包；安装版打开后会优先建立通向 CampusVision C1 的 SSH 隧道，并在软件内连接窗口显示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明；真实检索遇到 CampusVision C1 503 时也会打开同一个内嵌连接窗口并重试；SSH 隧道连接被远端重置时会被主进程吞掉并记录警告，不再弹出 Electron 主进程 JavaScript 错误；桌面端加载 `/demo` 时会清理 renderer cache，并附带 `asset=v0.1.28-ui` 页面参数，避免安装更新后继续显示旧 HTML/CSS/JS；桌面模式右上角提供 `检查更新` 入口，可在应用内下载新版并重启安装。
...
- The GKGuard C2 frontend lives in `backend/app/static/` and currently focuses on three screens: face search, face-search results, and person route map; the layout expands with the desktop window and avoids horizontal overflow down to `680x640`; the upload screen detects query-image faces, auto-searches when there is one face, and opens an enlarged original-image selection modal when there are multiple faces so the user can zoom and confirm the target; detection confidence is not person-match similarity, candidates below `0.65` but at least `0.45` stay visible with a low-confidence style and remain selectable, while candidates below `0.45` are not exposed as targets; if query-face detection fails, the UI stays on the upload screen instead of showing false successful results; if CampusVision C1 returns no matched records, times out, or fails during real search, the UI returns to an actionable upload state with a Chinese warning instead of staying stuck in loading; the result portrait prefers the selected query-face crop, the desktop result record list is restored to the left side and prefers CampusVision C1 face crops, detail panels render complete CampusVision C1 keyframes with a target-face box positioned against the rendered image content and the similarity label outside the box so it does not cover the face, and the keyframe preview dialog keeps the same overlay; result and route screens provide a return action to upload a new target; the mobile upload form is compact, action groups use two columns with about 42px touch height, mobile result and route record lists include horizontal-scroll hints and a right fade, the route screen shows route points, start, end, duration, and current-trajectory summary before the map on mobile, narrow-screen route-map labels stay inside the viewport with a compact legend, simulated keyframe decoration and route-map elements stay inside their containers at 390px, and the unified status feedback layer does not show empty toasts during initialization or screen switches.
- The Electron shell lives in `desktop/`; GitHub Actions can build the Windows installer. The packaged app prioritizes the SSH tunnel to CampusVision C1 and shows an embedded connection window with the server account, tunnel target, four-step progress, retry-on-failure state, and password-safety note when needed. It also opens the same embedded connection window and retries when real CampusVision C1 search returns 503, swallows remote tunnel reset errors as warnings instead of showing a main-process JavaScript error dialog, clears the renderer cache before loading `/demo`, appends `asset=v0.1.28-ui` to the page URL so installed updates do not reuse stale HTML/CSS/JS, and exposes a top-right `检查更新` entry in desktop mode for in-app update download and restart-to-install.
