...
本仓库集成了 GKGuard C2 的本地模拟演示、FastAPI 服务接口、Electron 桌面运行环境、CampusVision C1 真实检索接入、查询图目标人脸选择、结果关键帧标注、人物路线联动、Windows/macOS/Linux 桌面端发布、安全加固、协作流程管理和配套文档体系。当前最新版本是 `v0.1.36`。
...
- 当前稳定基线为 `v0.1.36`：主流程已覆盖本地 mock 演示和 CampusVision C1 真实检索两种模式，Windows/macOS/Linux 桌面端发布链路和本地开发后端都可进入 `/demo`。
...
- 桌面端已包含 CampusVision C1 自动连接、内嵌 SSH 密码窗口、SSH 主机密钥固定指纹自动比对、连接进度、ECONNRESET 抑制、页面缓存清理、静态资源版本参数和应用内更新入口；未配置固定指纹或指纹不匹配时会阻断连接，不再提供人工确认后继续连接的流程。
...
安装版内置默认 CampusVision C1 候选地址为本机 SSH 隧道 `http://127.0.0.1:18000`；校园网直连地址必须通过环境变量或 `%APPDATA%\GKGuard\c1-connection.json` 显式配置。打开应用后如果尚未通过隧道连接 CampusVision C1，会在软件内弹出服务器密码窗口；窗口会显示服务器账号、隧道目标、连接原因和四步连接进度。主进程会在发送服务器密码前自动比对 SSH 主机密钥固定指纹；服务器地址或主机密钥变化时，必须通过 `sshTunnel.hostFingerprint` 或 `C1_SSH_HOST_FINGERPRINT` 显式更新。未配置固定指纹或指纹不匹配时会阻断连接。连接失败时窗口会停留在当前界面并允许重新输入。密码只用于本次 SSH 连接，GKGuard 不保存、不落盘、不记录密码。
...
当前 `v0.1.36` 的基线结果：GKGuard C2 后端测试、CampusVision C1 服务端安全配置测试、查询图预处理测试、前端脚本、Electron 主进程和 preload 语法检查、桌面后端入口编译、Playwright E2E、`npm audit --audit-level=low` 和 `git diff --check` 通过；本地 Windows 打包继续生成 `GKGuard-Setup-0.1.36.exe`、`.blockmap` 和 `latest.yml`；Release workflow 会在 Windows、Intel macOS 和 Linux runner 上分别生成对应桌面端安装文件与 `latest*.yml` 元数据。本版在 `v0.1.35` 安全加固基础上继续收紧 GKGuard C2 / CampusVision C1 集成安全边界：内嵌 SSH 隧道取消人工确认主机指纹流程，改为固定指纹自动比对；CampusVision C1 精确保护视频列表、媒体、人物库、记录、相机和检索路径；人物库 HTML 预览改用服务端内联人脸图，避免受保护媒体接口被浏览器裸请求绕开；GKGuard C2 与 CampusVision C1 统一 API key 优先级；查询图解码超限返回 413 并清理临时文件；视频索引失败只回滚本次新增记录，保留既有成功索引。浏览器 E2E 继续覆盖本地模拟检索、CampusVision C1 单人自动检索、多人查询图放大弹窗选人、结果页人物照片按选中目标框坐标扩边裁切并填满方框安全区、低置信查询候选可见标注、查询图检测失败拦截、无匹配结果返回上传页、检索超时恢复、检索结果、桌面左侧记录列表、路线图、路线点定位、时间线定位、返回结果页、重新上传流程，以及模拟 CampusVision C1 媒体结果的关键帧预览弹窗、目标人脸框和框外相似度标注。
...
- The current stable baseline is `v0.1.36`: the main flow supports both local mock demonstration and CampusVision C1 real search, and the Windows/macOS/Linux desktop release flow plus the local development backend can enter `/demo`.
...
- The desktop app includes CampusVision C1 auto-connection, the embedded SSH password window, automatic pinned SSH host-key comparison, connection progress, ECONNRESET suppression, page-cache clearing, static asset version parameters, and the in-app update entry. If no pinned fingerprint is configured, or if the presented host key does not match, the connection is blocked instead of allowing manual approval.
...
The packaged app has one built-in CampusVision C1 candidate by default: the local SSH tunnel `http://127.0.0.1:18000`. Campus-network direct URLs must be explicitly configured through environment variables or `%APPDATA%\GKGuard\c1-connection.json`. If the tunnel is not connected, the app shows an embedded server-password window with account, tunnel target, connection reason, and four-step progress. Before sending the server password, the main process automatically compares the pinned SSH host-key fingerprint. If the server address or host key changes, update `sshTunnel.hostFingerprint` or `C1_SSH_HOST_FINGERPRINT` explicitly. Connections with no pinned fingerprint or a mismatched fingerprint are blocked. Failed connections stay in that window so the password can be re-entered. The password is used only for the current SSH session; GKGuard does not store it, write it to disk, or log it.
...
Current `v0.1.36` baseline: GKGuard C2 backend tests, CampusVision C1 service-side security configuration tests, query-face preprocessing tests, frontend script, Electron main-process and preload syntax checks, desktop backend entrypoint compilation, Playwright E2E, `npm audit --audit-level=low`, and `git diff --check` pass. The local Windows build still generates `GKGuard-Setup-0.1.36.exe`, `.blockmap`, and `latest.yml`; the Release workflow generates platform-specific desktop packages and `latest*.yml` metadata on Windows, Intel macOS, and Linux runners. Building on the `v0.1.35` hardening, this release further tightens the GKGuard C2 / CampusVision C1 integration boundary: embedded SSH no longer allows manual host-fingerprint approval and instead requires automatic pinned-fingerprint comparison; CampusVision C1 precisely protects video-list, media, person-index, record, camera, and search paths; the person-gallery HTML preview uses server-side inline face images so protected media endpoints are not requested naked by the browser; GKGuard C2 and CampusVision C1 share the same API-key precedence; oversized decoded query images return 413 and clean up temporary uploads; and failed video indexing rolls back only records created by the current attempt while preserving existing successful indexes. Browser E2E continues to cover mock search, CampusVision C1 single-face auto-search, enlarged multi-face query selection, result portraits cropped with padding from the selected target-box coordinates and filling the portrait safe area, visible low-confidence query candidates, query-face detection failure blocking, no-match upload recovery, search timeout recovery, results, the left-side desktop record list, route map, route-point locate, timeline locate, return to results, re-upload, a mocked CampusVision C1 media keyframe preview dialog, target-face overlays, and outside-box similarity labels.
