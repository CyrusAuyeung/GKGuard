...
- Pull Request 合并前需要 `.github/workflows/ci.yml` 中的 `Verify` 检查通过，完成审查，解决所有对话，并检查 PR 正文 reaction、Issue 评论、审查线程和审查状态。👀 / 👍 这类审查状态 reaction 只存在且只读取 PR 正文下方的 reaction，不存在于 review 后评论，也不以 review/comment 下方 reaction 作为审查状态。没有任何审查信号时应继续等待或轮询；只有在没有 PR 正文审查信号且需要主动启动审查时，才可评论 `@codex review`；PR 正文已有 👀 时不要再次手动触发 review，只等待结果。PR 正文 👀 表示仍在 review，不要合并；PR 正文 👍、approval 或明确无阻断结论表示当前 review 已完成；Issue 评论、review 评论或未解决线程表示需要处理反馈。仓库使用 squash merge，合并后删除源分支。
...
- Before merge, Pull Requests must pass the `Verify` check from `.github/workflows/ci.yml`, complete review, resolve all conversations, and check PR-body reactions, issue comments, review threads, and review state. Review-status reactions such as 👀 / 👍 exist only below the PR body and only PR-body reactions count; reactions below review comments or other comments are not review-status signals. If no review signal is present, keep waiting or polling; only comment `@codex review` when there is no PR-body review signal and a review needs to be started actively; if the PR body already has 👀, do not manually retrigger review and wait for the result. PR-body 👀 means review is still in progress and the PR must not be merged; PR-body 👍, approval, or an explicit no-blocker conclusion means the current review is complete; issue comments, review comments, or unresolved threads mean feedback must be addressed. The repository uses squash merge and deletes merged head branches.
