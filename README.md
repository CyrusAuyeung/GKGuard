...
- **CampusVision C1 真实检索接入**：GKGuard C2 通过 `/c1/...` 代理探测 CampusVision C1 候选地址，转发查询图检测和以图搜人请求，改写关键帧/人脸裁剪图媒体 URL，对成功媒体响应按实际响应地址、API key、候选配置和连接代次隔离缓存并限制内存占用，媒体请求发起前重新校验连接代次，对短期健康状态探测做进程内复用与失败失效处理，把 CampusVision C1 响应归一化成 GKGuard C2 前端可直接展示的记录、相似度、关键帧和轨迹模型。
...
- GKGuard C2 本地代理层：在 GKGuard C2 后端内探测 CampusVision C1 服务候选地址，检查 `/openapi.json` 与 `/health`，转发以图搜人请求，改写媒体 URL，对成功媒体响应按实际响应地址、API key、候选配置和连接代次隔离缓存并限制内存占用，媒体请求发起前重新校验连接代次，对短期健康状态探测做进程内复用与失败失效处理，把 CampusVision C1 响应归一化为 GKGuard C2 视图模型。
...
- GKGuard C2 的 CampusVision C1 代理已对成功读取的关键帧/人脸裁剪图使用进程内短期缓存，缓存键包含实际响应的 CampusVision C1 地址、API key、候选地址配置和连接代次，并设置条目数、总字节和单项字节上限；实时状态确认或桌面端 SSH 隧道重新确认会清理上一连接代次的媒体缓存，媒体请求发起前若连接代次变化会重新解析候选或返回可重试错误，媒体请求返回前若连接代次已变化则不会写入新缓存或回写选中地址；同一次媒体请求内从失效候选回退到可用候选时，会保留成功候选并写入对应代次缓存，普通请求会复用短期 CampusVision C1 健康状态探测结果。
...
- Pull Request 合并前需要 `.github/workflows/ci.yml` 中的 `Verify` 检查通过，完成审查，解决所有对话，并检查 PR 正文 reaction、Issue 评论、审查线程和审查状态。👀 / 👍 这类状态表情只以 PR 正文 reaction 为准，不以 review 后的评论 reaction 为准。没有任何审查信号时应继续等待或轮询；PR 正文 👀 表示仍在 review，不要合并；PR 正文 👍、approval 或明确无阻断结论表示当前 review 已完成；Issue 评论、review 评论或未解决线程表示需要处理反馈。仓库使用 squash merge，合并后删除源分支。
...
- **CampusVision C1 real-search integration**: GKGuard C2 probes CampusVision C1 candidate URLs through `/c1/...`, forwards query-face detection and image-search requests, rewrites keyframe/face-crop media URLs, caches successful media responses by responding address, API key, candidate configuration, and connection generation with memory limits, re-checks connection generation before sending media requests, reuses short-lived healthy status probes in process, evicts stale healthy status after failed fresh probes, and normalizes CampusVision C1 responses into records, similarity values, keyframes, and trajectory models that the GKGuard C2 UI can render directly.
...
- GKGuard C2 local proxy layer: inside the GKGuard C2 backend, probes CampusVision C1 service candidate URLs, checks `/openapi.json` and `/health`, forwards image-search requests, rewrites media URLs, caches successful media responses by responding address, API key, candidate configuration, and connection generation with memory limits, re-checks connection generation before sending media requests, reuses short-lived healthy status probes in process, evicts stale healthy status after failed fresh probes, and normalizes CampusVision C1 responses into GKGuard C2 view models.
...
- The GKGuard C2 CampusVision C1 proxy uses a short-lived in-process cache for successfully fetched keyframes and face crops, scoped by the responding CampusVision C1 address, API key, candidate-URL configuration, and connection generation, with item-count, total-byte, and per-item byte limits. Fresh status confirmation or desktop SSH-tunnel reconfirmation clears media cache entries from the previous connection generation; media requests re-resolve candidates or return a retryable error if the generation changes before the GET is sent, and media responses are not written into the new cache or allowed to overwrite the selected address if the connection generation changes before the response returns. When the same media request falls back from a failed candidate to a healthy candidate, the successful candidate is preserved and cached in its resulting generation, while normal requests reuse short-lived healthy CampusVision C1 status probes.
...
- Before merge, Pull Requests must pass the `Verify` check from `.github/workflows/ci.yml`, complete review, resolve all conversations, and check PR-body reactions, issue comments, review threads, and review state. Status emoji such as 👀 / 👍 are authoritative only as PR-body reactions, not as reactions on post-review comments. If no review signal is present, keep waiting or polling; PR-body 👀 means review is still in progress and the PR must not be merged; PR-body 👍, approval, or an explicit no-blocker conclusion means the current review is complete; issue comments, review comments, or unresolved threads mean feedback must be addressed. The repository uses squash merge and deletes merged head branches.
