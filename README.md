...
GKGuard 是校园安防 AI 检索项目中的 GKGuard C2 工作台：负责桌面界面、本地后端、检索结果展示、路线可视化、本地模拟回退、审计和外部服务代理。CampusVision C1 是独立的视频检索服务：负责视频上传、抽帧、人脸向量、人物库、以图搜人、关键帧和轨迹输出。当前 `v0.1.33` 状态下，仓库包含 GKGuard C2 后端、三屏人脸检索前端、Electron 桌面壳、CampusVision C1 自动连接/内嵌 SSH 密码窗口和连接进度、查询图人脸检测预处理与重试、单人自动检索、多人显式放大选人按钮、放大弹窗选人、低置信查询候选可见标注、选中人脸检索、结果页人物照片按选中目标框坐标扩边裁切、优先调整为方形裁切、靠边平移保留完整人脸并填满人物照片安全显示区、查询图检测失败拦截、无匹配结果回到上传页、检索超时恢复、结果关键帧目标框和框外相似度标注、响应式宽屏布局、完整显示上传图、CampusVision C1 人脸裁剪缩略图和 CampusVision C1 关键帧的媒体展示、关键帧预览弹窗、桌面结果页左侧记录列表、路线点/时间线定位联动、空状态与键盘焦点状态、移动端上传页和操作按钮密度优化、移动端结果/路线记录横向滚动提示、移动端路线当前轨迹摘要、窄屏路线图标签边缘避让和紧凑图例、统一且不会空白显示的状态提示、Electron 页面缓存刷新、旧后台资源复用保护、Playwright 浏览器 E2E 回归、GKGuard Roadmap 半自动字段脚本、CampusVision C1 远端半自动部署脚本、经过细节修正的线性功能图标、品牌图标文件、软件内下载并重启安装的更新入口，以及作为独立模块导入的 CampusVision C1 服务源码。
...
- GKGuard C2 前端位于 `backend/app/static/`，当前主流程为 `人脸检索`、`人脸检索结果`、`人物路线图` 三个界面；页面宽度会随桌面窗口扩展，支持 `680x640` 小窗口下无横向溢出；上传页会先检测查询图人脸，单人图自动选中并检索，多人图显示可点击人脸框，并提供显式“放大选择目标人脸”按钮；用户也可直接点击上传区重新选择文件，开始检索时若未选目标仍会打开弹窗要求确认；弹窗默认按容器内可见宽高最大化适配原图，点击 `适应` 会回到无需滚动即可完整查看的最大尺寸，同时仍支持继续缩小/放大；检测置信度不等同于人物相似度，低于 `0.65` 但不低于 `0.45` 的候选会以低置信样式显示并仍可选择，低于 `0.45` 的候选不作为可选目标；若查询图人脸检测失败，不再展示伪成功结果，而是停留在上传页提示重试或检查 CampusVision C1；若 CampusVision C1 返回无匹配记录、请求超时或真实检索失败，页面会回到可继续操作的上传状态并显示中文提示，不再卡在“检索中”；结果页人物照片优先显示选人阶段目标框扩边后的查询人脸裁切图，空间允许时会调整为方形裁切，裁切框接近图像边缘时会向内平移以保留完整人脸，图片元素固定在人物照片方框的 8px 安全边距内并用 `object-fit: contain` 充分利用方框空间，缺失时才回退 CampusVision C1 返回的人脸缩略图；桌面端记录列表恢复在左侧纵向展示并优先使用 CampusVision C1 人脸裁剪缩略图，详情区完整展示 CampusVision C1 关键帧并按实际图片内容区域定位目标人脸框，匹配相似度显示在框外，避免遮挡人脸、落在左上角或被黑边偏移，关键帧预览弹窗也保留同样标注；结果页和路线页都可一键返回重新上传；移动端上传页压缩表单和上传区高度，结果页和路线页操作按钮使用约 42px 触控高度的双列紧凑布局；移动端结果页和路线页记录列表提供横向滚动提示与右侧渐隐；路线页工具栏将 `重新上传` 固定在最左侧，并在地图前展示轨迹点、起点、终点、总时长和当前轨迹摘要，路线图点位、关联记录和时间线可联动定位当前记录，窄屏地图标签会避让边缘且图例压缩为横向条；390px 视口下模拟关键帧装饰和路线地图元素保持在容器内；检索、CampusVision C1 回退、定位、导出和更新入口使用统一状态提示，初始和切屏状态不会显示空白 toast。
- Electron 桌面壳位于 `desktop/`，GitHub Actions 可构建 Windows 安装包；安装版打开后会优先建立通向 CampusVision C1 的 SSH 隧道，并在软件内连接窗口显示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明；真实检索遇到 CampusVision C1 503 时也会打开同一个内嵌连接窗口并重试；SSH 隧道连接被远端重置时会被主进程吞掉并记录警告，不再弹出 Electron 主进程 JavaScript 错误；桌面端加载 `/demo` 时会清理 renderer cache，并附带 `asset=v0.1.33-ui` 页面参数，避免安装更新后继续显示旧 HTML/CSS/JS；桌面模式右上角提供 `检查更新` 入口，可在应用内下载新版并重启安装。
...
- 未上传图片时，GKGuard C2 前端可直接使用本地模拟记录演示三屏流程；上传图片后，GKGuard C2 会先检测查询图人脸并尝试真实 CampusVision C1 检索，检测或检索失败时停留在上传页提示重试，不再展示伪成功结果。
...
当前 `v0.1.33` 的基线结果：后端测试 `27 passed`，前端脚本、Electron 主进程和 preload 语法检查通过，桌面后端入口编译通过，`npm run test:e2e` 通过 21 个 Chromium 用例，CampusVision C1 查询图预处理测试在缺少 OpenCV 的本地环境中按测试级跳过，`npm audit --audit-level=low` 无漏洞，`git diff --check` 通过，`npm run dist` 本地生成 `GKGuard-Setup-0.1.33.exe`、`.blockmap` 和 `latest.yml`；本版未改动远端 CampusVision C1 服务或远端半自动部署脚本。浏览器 E2E 覆盖 `1280x720`、`680x640` 和 `390x720` 视口下的本地模拟检索、CampusVision C1 单人自动检索、多人查询图放大弹窗选人、结果页人物照片按选中目标框坐标扩边裁切并填满方框安全区、低置信查询候选可见标注、查询图检测失败拦截、无匹配结果返回上传页、检索超时恢复、检索结果、桌面左侧记录列表、路线图、路线点定位、时间线定位、返回结果页、重新上传流程，以及模拟 CampusVision C1 媒体结果的关键帧预览弹窗、目标人脸框和框外相似度标注，并检查无页面级横向溢出、无空白 toast、浏览器 console warning/error 为 0；GitHub Actions CI 工作流覆盖 PR 基础检查，发布工作流会生成 Windows 安装包和 Electron 自动更新元数据。
...
GKGuard is the GKGuard C2 workbench for the campus security AI search project: it owns the desktop UI, local backend, result presentation, route visualization, mock fallback, audit features, and external-service proxy layer. CampusVision C1 is the separate video-search service for video upload, frame sampling, face embeddings, person indexing, image-based person search, keyframes, and trajectory output. As of `v0.1.33`, this repository contains the GKGuard C2 backend, the three-screen face-search frontend, the Electron desktop shell, CampusVision C1 auto-connection with an embedded SSH password prompt and connection progress, query-face preprocessing and retry, single-face auto-search, explicit enlarged face-selection button, enlarged multi-face selection modal, visible low-confidence query candidates, selected-face search, result portraits cropped with padding from the selected target-box coordinates, made square when space allows, shifted inward near image edges, and fitted into the portrait-frame safe area, query-face detection failure blocking, idle upload recovery for no-match results, search timeout recovery, target-face boxes plus outside-box similarity labels on result keyframes, responsive wide-window layout, full-display media handling for uploaded targets, CampusVision C1 face-crop thumbnails, CampusVision C1 keyframes, a keyframe preview dialog, a left-side desktop result record list, route-point/timeline selection sync, empty states and keyboard focus states, mobile upload/action density improvements, mobile horizontal-scroll hints for result and route records, a mobile current-trajectory summary, narrow-screen route-map label edge avoidance with a compact legend, unified status feedback that does not show empty toasts, Electron renderer cache refresh, stale-backend asset reuse protection, Playwright browser E2E regression, a GKGuard Roadmap field helper script, a semi-automatic CampusVision C1 remote deployment script, polished linear UI icons, brand icon assets, an in-app update flow that downloads and restarts to install updates, and the imported CampusVision C1 service source as a separate module.
...
- The GKGuard C2 frontend lives in `backend/app/static/` and currently focuses on three screens: face search, face-search results, and person route map; the layout expands with the desktop window and avoids horizontal overflow down to `680x640`; the upload screen detects query-image faces, auto-searches when there is one face, and shows clickable face boxes plus an explicit enlarged face-selection button when there are multiple faces; clicking the upload area itself still opens the file picker, starting search without a selected target opens the modal, and the modal defaults to the largest no-scroll full-image fit allowed by the visible frame, with `适应` returning to that maximum fit while zoom-out and zoom-in remain available; detection confidence is not person-match similarity, candidates below `0.65` but at least `0.45` stay visible with a low-confidence style and remain selectable, while candidates below `0.45` are not exposed as targets; if query-face detection fails, the UI stays on the upload screen instead of showing false successful results; if CampusVision C1 returns no matched records, times out, or fails during real search, the UI returns to an actionable upload state with a Chinese warning instead of staying stuck in loading; the result portrait prefers a padded crop from the selected target box, makes the crop square when the source image has enough room, shifts the crop inward near image edges to preserve the full face, fixes the image element inside an 8px safe area with `object-fit: contain` so the frame space is used consistently, and falls back to the CampusVision C1 face thumbnail only when needed; the desktop result record list is restored to the left side and prefers CampusVision C1 face crops, detail panels render complete CampusVision C1 keyframes with a target-face box positioned against the rendered image content and the similarity label outside the box so it does not cover the face, and the keyframe preview dialog keeps the same overlay; result and route screens provide a return action to upload a new target; the mobile upload form is compact, action groups use two columns with about 42px touch height, mobile result and route record lists include horizontal-scroll hints and a right fade, the route screen shows route points, start, end, duration, and current-trajectory summary before the map on mobile, narrow-screen route-map labels stay inside the viewport with a compact legend, simulated keyframe decoration and route-map elements stay inside their containers at 390px, and the unified status feedback layer does not show empty toasts during initialization or screen switches.
- The Electron shell lives in `desktop/`; GitHub Actions can build the Windows installer. The packaged app prioritizes the SSH tunnel to CampusVision C1 and shows an embedded connection window with the server account, tunnel target, four-step progress, retry-on-failure state, and password-safety note when needed. It also opens the same embedded connection window and retries when real CampusVision C1 search returns 503, swallows remote tunnel reset errors as warnings instead of showing a main-process JavaScript error dialog, clears the renderer cache before loading `/demo`, appends `asset=v0.1.33-ui` to the page URL so installed updates do not reuse stale HTML/CSS/JS, and exposes a top-right `检查更新` entry in desktop mode for in-app update download and restart-to-install.
...
- When no image is uploaded, the frontend can use local mock records to demonstrate the three-screen flow. After an image is uploaded, the frontend detects query faces and attempts real CampusVision C1 search; if detection fails, search fails, or CampusVision C1 returns no matched records, it stays on the upload screen with a retry/error message instead of showing false successful results.
...
Current `v0.1.33` baseline: backend tests pass with `27 passed`, frontend script, Electron main-process, and preload syntax checks pass, the desktop backend entrypoint compiles, `npm run test:e2e` passes 21 Chromium cases, CampusVision C1 query preprocessing tests skip locally when OpenCV is unavailable, `npm audit --audit-level=low` reports no vulnerabilities, `git diff --check` passes, and `npm run dist` locally generates `GKGuard-Setup-0.1.33.exe`, `.blockmap`, and `latest.yml`. This release does not modify the remote CampusVision C1 service or the semi-automatic remote deployment script. Browser E2E covers mock search, CampusVision C1 single-face auto-search, enlarged multi-face query selection, result portraits cropped with padding from the selected target-box coordinates and filling the portrait safe area, visible low-confidence query candidates, query-face detection failure blocking, no-match upload recovery, search timeout recovery, results, the left-side desktop record list, route map, route-point locate, timeline locate, return to results, re-upload, a mocked CampusVision C1 media keyframe preview dialog, target-face overlays, and outside-box similarity labels at `1280x720`, `680x640`, and `390x720`, with no page-level horizontal overflow, empty toast, or browser console warning/error. The GitHub Actions CI workflow covers PR checks, and the Release workflow produces the Windows installer and Electron auto-update metadata.
