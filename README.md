...
- Pull Request 合并前需要 `.github/workflows/ci.yml` 中的 `Verify` 检查通过，完成审查，解决所有对话，并检查关键审查评论 reaction：👀 表示仍在 review，不要合并；👍、approval 或明确无阻断结论表示当前 review 已完成。仓库使用 squash merge，合并后删除源分支。
...
- Before merge, Pull Requests must pass the `Verify` check from `.github/workflows/ci.yml`, complete review, resolve all conversations, and check key review-comment reactions: 👀 means review is still in progress and the PR must not be merged; 👍, approval, or an explicit no-blocker conclusion means the current review is complete. The repository uses squash merge and deletes merged head branches.
