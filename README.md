...
当前 `v0.1.32` 的基线结果：后端测试 `27 passed`，前端脚本、Electron 主进程和 preload 语法检查通过，桌面后端入口编译通过，`npm run test:e2e` 通过 18 个 Chromium 用例，CampusVision C1 查询图预处理测试在缺少 OpenCV 的本地环境中按测试级跳过，远端 CampusVision C1 conda 环境测试通过，CampusVision C1 远端半自动部署脚本可完成更新、依赖检查、uvicorn 重启、`/health` 检查和 `query-faces` OpenAPI 检查，`npm audit --audit-level=low` 无漏洞，`git diff --check` 通过；浏览器 E2E 覆盖 `1280x720`、`680x640` 和 `390x720` 视口下的本地模拟检索、CampusVision C1 单人自动检索、多人查询图放大弹窗选人、结果页人物照片按选中目标框坐标裁切、低置信查询候选可见标注、查询图检测失败拦截、无匹配结果返回上传页、检索超时恢复、检索结果、桌面左侧记录列表、路线图、路线点定位、时间线定位、返回结果页、重新上传流程，以及模拟 CampusVision C1 媒体结果的关键帧预览弹窗、目标人脸框和框外相似度标注，并检查无页面级横向溢出、无空白 toast、浏览器 console warning/error 为 0；GitHub Actions CI 工作流覆盖 PR 基础检查，发布工作流会生成 Windows 安装包和 Electron 自动更新元数据。
...
Current `v0.1.32` baseline: backend tests pass with `27 passed`, frontend script, Electron main-process, and preload syntax checks pass, the desktop backend entrypoint compiles, `npm run test:e2e` passes 18 Chromium cases, CampusVision C1 query preprocessing tests skip locally when OpenCV is unavailable, the remote CampusVision C1 conda environment tests pass, the semi-automatic CampusVision C1 remote deployment script can update the repo, check dependencies, restart uvicorn, verify `/health`, and verify `query-faces` in OpenAPI, `npm audit --audit-level=low` reports no vulnerabilities, and `git diff --check` passes. Browser E2E covers mock search, CampusVision C1 single-face auto-search, enlarged multi-face query selection, result portraits cropped from the selected target-box coordinates, visible low-confidence query candidates, query-face detection failure blocking, no-match upload recovery, search timeout recovery, results, the left-side desktop record list, route map, route-point locate, timeline locate, return to results, re-upload, a mocked CampusVision C1 media keyframe preview dialog, target-face overlays, and outside-box similarity labels at `1280x720`, `680x640`, and `390x720`, with no page-level horizontal overflow, empty toast, or browser console warning/error. The GitHub Actions CI workflow covers PR checks, and the Release workflow produces the Windows installer and Electron auto-update metadata.
