...
- **CampusVision C1 真实检索接入**：GKGuard C2 通过 `/c1/...` 代理探测 CampusVision C1 候选地址，转发查询图检测和以图搜人请求，改写关键帧/人脸裁剪图媒体 URL，对成功媒体响应按 CampusVision C1 base URL 隔离缓存，并对短期健康状态探测做进程内复用与失败失效处理，把 CampusVision C1 响应归一化成 GKGuard C2 前端可直接展示的记录、相似度、关键帧和轨迹模型。
...
- GKGuard C2 本地代理层：在 GKGuard C2 后端内探测 CampusVision C1 服务候选地址，检查 `/openapi.json` 与 `/health`，转发以图搜人请求，改写媒体 URL，对成功媒体响应按 CampusVision C1 base URL 隔离缓存，并对短期健康状态探测做进程内复用与失败失效处理，把 CampusVision C1 响应归一化为 GKGuard C2 视图模型。
...
- GKGuard C2 的 CampusVision C1 代理已对成功读取的关键帧/人脸裁剪图使用进程内短期缓存，缓存键包含当前 CampusVision C1 base URL，并复用短期 CampusVision C1 健康状态探测结果；实时状态探测失败或身份校验失败时会清理对应地址的旧健康缓存。
...
- **CampusVision C1 real-search integration**: GKGuard C2 probes CampusVision C1 candidate URLs through `/c1/...`, forwards query-face detection and image-search requests, rewrites keyframe/face-crop media URLs, caches successful media responses by CampusVision C1 base URL, reuses short-lived healthy status probes in process, evicts stale healthy status after failed fresh probes, and normalizes CampusVision C1 responses into records, similarity values, keyframes, and trajectory models that the GKGuard C2 UI can render directly.
...
- GKGuard C2 local proxy layer: inside the GKGuard C2 backend, probes CampusVision C1 service candidate URLs, checks `/openapi.json` and `/health`, forwards image-search requests, rewrites media URLs, caches successful media responses by CampusVision C1 base URL, reuses short-lived healthy status probes in process, evicts stale healthy status after failed fresh probes, and normalizes CampusVision C1 responses into GKGuard C2 view models.
...
- The GKGuard C2 CampusVision C1 proxy uses a short-lived in-process cache for successfully fetched keyframes and face crops, scoped by CampusVision C1 base URL, and reuses short-lived healthy CampusVision C1 status probes. Failed fresh status probes or identity checks clear stale healthy cache entries for the affected address.
