...
- **人物特征检索前端流程**：当前版本包含人物特征检索入口，可按上装颜色、眼镜状态、外观倾向、摄像头和时间范围查询 CampusVision C1 事件；结果复用检索记录、关键帧、人脸框、相似度、路线联动和导出展示模型。`v0.2.1` 在 `v0.2.0` 集成基础上修复路线顺序、路线点到结果记录的稳定映射、重复索引、人物索引与 appearance session 重建和查询图异常清理问题。
