...
- **人物特征检索前端流程**：当前版本包含人物特征检索入口，可按上装颜色、眼镜状态、外观倾向、摄像头和时间范围查询 CampusVision C1 事件；结果复用检索记录、关键帧、人脸框、相似度、路线联动和导出展示模型。`v0.2.2` 延续 `v0.2.0` 功能集成和 `v0.2.1` 稳定性修复，并补齐 review 后续：查询图候选接口参数位置、4xx 校验错误传递、路线点唯一高亮和未映射记录的路线高亮边界。
