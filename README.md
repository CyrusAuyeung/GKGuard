...
GKGuard 是校园安防 AI 检索项目中的 C2 演示端。当前 `v0.1.17` 状态下，仓库包含 C2 后端、三界面人脸检索前端、Electron 桌面壳、C1 自动连接/内嵌 SSH 密码窗口和连接进度、经过细节修正的线性功能图标、品牌图标资产、软件内下载并重启安装的更新入口，以及作为独立模块导入的 C1 CampusVision 视频检索服务。

## 版本状态与交付结论

- 当前最新版本：`v0.1.17`。
- 当前阶段结论：C2 个人可控范围已经形成可交付版本，可以用于安装演示、C1 联调、离线 mock 演示和项目交接。
- 发布状态：Windows 安装包、`.blockmap` 和 `latest.yml` 已由 GitHub Actions 发布到 `v0.1.17` Release。
- 交付形态：普通演示用户使用 Windows 安装包；开发和排障人员可运行 FastAPI 后端、静态演示页和 Electron 桌面壳。
- 后续主要工作不再是 C2 壳体核心功能，而是 C1 真实数据稳定性、CampusCar/UE 外部服务接入、最终答辩验收和按反馈 polish。

## 交付范围

当前仓库覆盖以下内容：

- C2 FastAPI 后端：健康检查、人员/车辆 mock API、事件处置、审计、C1 代理和 CampusCar/UE 占位接口。
- C2 静态前端：人脸检索、检索结果、人物路线图三屏流程，支持上传、重新上传、结果查看、路线查看和导出入口。
- C1 适配层：候选地址探测、`/openapi.json` 与 `/health` 检查、以图搜人转发、媒体 URL 改写和 C2 视图模型归一化。
- Electron 桌面壳：启动内置 C2 后端、显示启动页、加载演示页、内嵌 C1 SSH 密码窗口、连接进度和应用内更新。
- 发布工程：GitHub Actions 自动构建 Windows 安装包，生成 Release notes，并上传安装包和 Electron 自动更新元数据。
- 文档与治理：双语 README、API 合同、C1/C2 集成说明、自动连接说明、演示脚本、数据字典、发布说明、安全/隐私/贡献/支持声明。

## 当前不包含

- 不包含真实 CampusCar 控制、ROS2 节点、UE 运行时或底盘控制逻辑。
- 不包含真实 C1 运行数据、真实视频、真实查询图片、SQLite 数据库、模型缓存或服务器 `.env`。
- 不把服务器密码、SSH 私钥、token 或 API key 写入仓库、配置文件或日志。
- 不承诺在离线或无校园网/VPN/SSH 隧道环境中完成真实 C1 检索；离线时使用 mock fallback 演示 C2 壳体流程。

## 当前状态

- C2 后端位于 `backend/`，提供 FastAPI API、静态演示页、C1 代理、mock fallback、审计和 CampusCar/UE 占位接口。
- C2 前端位于 `backend/app/static/`，当前主流程为 `人脸检索`、`人脸检索结果`、`人物路线图` 三个界面；上传页和结果页会优先完整显示用户上传的目标照片，记录列表展示 C1 关键帧缩略图，结果页和路线页都可一键返回重新上传，路线页工具栏将 `重新上传` 固定在最左侧。
- Electron 桌面壳位于 `desktop/`，GitHub Actions 可构建 Windows 安装包；安装版打开后会优先建立 SSH 隧道并在软件内提示输入服务器密码与连接进度，真实检索遇到 C1 503 时也会打开同一个内嵌连接窗口并重试；桌面模式右上角提供 `检查更新` 入口，可在应用内下载新版并重启安装。
...
Windows Desktop App (Electron)
  -> bundled local C2 backend (FastAPI)
  -> /demo static UI
...
C2 前端只访问 C2 后端。真实 C1 检索、C1 媒体帧、CampusCar/UE 占位能力都通过 C2 后端合同暴露，避免前端直接依赖多个外部服务。
...
- [docs/data_dictionary.md](docs/data_dictionary.md)：mock 数据、C1 检索结果和 CampusCar/UE 占位字段。
...
当前 `v0.1.17` 已验证：后端测试 `24 passed`，Electron 主进程和 preload 语法检查通过，`npm audit --audit-level=low` 为 0 vulnerabilities，本地 `npm run dist` 可生成 Windows 安装包。
...
GKGuard is the C2 demo shell for the campus security AI search project. As of `v0.1.17`, this repository contains the C2 backend, the three-screen face-search frontend, the Electron desktop shell, C1 auto-connection with an embedded SSH password prompt and connection progress, polished linear UI icons, brand icon assets, an in-app update flow that downloads and restarts to install updates, and the imported C1 CampusVision video retrieval service as a separate module.

## Release Status And Delivery Conclusion

- Latest version: `v0.1.17`.
- Current-stage conclusion: the C2 owner-controlled scope is in a deliverable state for installer demos, C1 integration testing, offline mock demos, and project handoff.
- Release status: the Windows installer, `.blockmap`, and `latest.yml` have been published by GitHub Actions under the `v0.1.17` Release.
- Delivery form: demo users should use the Windows installer; developers and troubleshooters can run the FastAPI backend, static demo page, and Electron shell locally.
- Remaining work is mainly C1 data/service stability, external CampusCar/UE integration, final review acceptance, and feedback-driven polish rather than core C2 shell functionality.

## Delivery Scope

This repository currently covers:

- C2 FastAPI backend: health checks, people/vehicle mock APIs, event disposition, audit, C1 proxy, and CampusCar/UE placeholder endpoints.
- C2 static frontend: face search, search results, and person route map screens with upload, re-upload, result review, route review, and export entry points.
- C1 adapter layer: candidate URL probing, `/openapi.json` and `/health` checks, image-search forwarding, media URL rewriting, and C2 view-model normalization.
- Electron desktop shell: bundled C2 backend startup, loading page, demo page loading, embedded C1 SSH password window, connection progress, and in-app updates.
...
- Documentation and governance: bilingual README, API contract, C1/C2 integration notes, auto-connection guide, demo script, data dictionary, release notes, and security/privacy/contribution/support statements.

## Out Of Scope

- Real CampusCar control, ROS2 nodes, UE runtime, and chassis-control logic are not included.
- Real C1 runtime data, real videos, real query images, SQLite databases, model caches, and server `.env` files are not included.
- Server passwords, SSH private keys, tokens, and API keys must not be written to the repository, config files, or logs.
- Real C1 search is not guaranteed offline or without campus network, VPN, or SSH tunnel access; offline mode uses mock fallback to demonstrate the C2 shell flow.
...
- The C2 frontend lives in `backend/app/static/` and currently focuses on three screens: face search, face-search results, and person route map; the upload and result screens prefer the full uploaded target image, render C1 keyframe thumbnails in the record list, provide a return action to upload a new target from result or route screens, and keep the route-screen `重新上传` action as the leftmost toolbar control.
- The Electron shell lives in `desktop/`; GitHub Actions can build the Windows installer. The packaged app prioritizes the SSH tunnel, prompts for the server SSH password and connection progress inside the app when needed, also opens the same embedded connection window and retries when real C1 search returns 503, and exposes a top-right `检查更新` entry in desktop mode for in-app update download and restart-to-install.
...
The C2 frontend calls only the C2 backend. Real C1 search, C1 media frames, and CampusCar/UE placeholder capabilities are exposed through C2 backend contracts so the frontend does not directly depend on multiple external services.
...
- [docs/data_dictionary.md](docs/data_dictionary.md): mock data, C1 search results, and CampusCar/UE placeholder fields.
...
Current `v0.1.17` validation: backend tests pass with `24 passed`, Electron main/preload syntax checks pass, `npm audit --audit-level=low` reports 0 vulnerabilities, and local `npm run dist` produces the Windows installer.
