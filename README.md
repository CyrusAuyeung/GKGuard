...
GKGuard 是校园安防 AI 检索项目中的 GKGuard C2 工作台：负责桌面界面、本地后端、检索结果展示、路线可视化、本地模拟回退、审计和外部服务代理。CampusVision C1 是独立的视频检索服务：负责视频上传、抽帧、人脸向量、人物库、以图搜人、关键帧和轨迹输出。当前 `v0.1.26` 状态下，仓库包含 GKGuard C2 后端、三屏人脸检索前端、Electron 桌面壳、CampusVision C1 自动连接/内嵌 SSH 密码窗口和连接进度、查询图人脸检测、单人自动检索、多人原图框选、低置信查询框过滤、选中人脸检索、查询图检测失败拦截、无匹配结果回到上传页、结果关键帧目标框和相似度标注、响应式宽屏布局、完整显示上传图、CampusVision C1 人脸裁剪缩略图和 CampusVision C1 关键帧的媒体展示、关键帧预览弹窗、桌面结果页左侧记录列表、路线点/时间线定位联动、空状态与键盘焦点状态、移动端上传页和操作按钮密度优化、移动端结果/路线记录横向滚动提示、移动端路线当前轨迹摘要、窄屏路线图标签边缘避让和紧凑图例、统一且不会空白显示的状态提示、Electron 页面缓存刷新、旧后台资源复用保护、Playwright 浏览器 E2E 回归、GKGuard Roadmap 半自动字段脚本、经过细节修正的线性功能图标、品牌图标文件、软件内下载并重启安装的更新入口，以及作为独立模块导入的 CampusVision C1 服务源码。
...
- GKGuard C2 前端位于 `backend/app/static/`，当前主流程为 `人脸检索`、`人脸检索结果`、`人物路线图` 三个界面；页面宽度会随桌面窗口扩展，支持 `680x640` 小窗口下无横向溢出；上传页会先检测查询图人脸，单人图自动选中并检索，多人图在原图上显示人脸框和检测置信度，存在高置信候选时会隐藏低于 `0.65` 的低置信检测框，用户选择目标后只检索选中人脸；若查询图人脸检测失败，不再展示伪成功结果，而是停留在上传页提示重试或检查 CampusVision C1；若 CampusVision C1 返回无匹配记录，页面会回到可继续操作的上传状态并显示中文提示，不再卡在“检索中”；结果页人物照片优先显示选中的查询人脸裁切图，桌面端记录列表恢复在左侧纵向展示并优先使用 CampusVision C1 人脸裁剪缩略图，详情区完整展示 CampusVision C1 关键帧并按实际图片内容区域定位目标人脸框和相似度，避免目标框落在左上角或被黑边偏移，关键帧预览弹窗也保留同样标注；结果页和路线页都可一键返回重新上传；移动端上传页压缩表单和上传区高度，结果页和路线页操作按钮使用约 42px 触控高度的双列紧凑布局；移动端结果页和路线页记录列表提供横向滚动提示与右侧渐隐；路线页工具栏将 `重新上传` 固定在最左侧，并在地图前展示轨迹点、起点、终点、总时长和当前轨迹摘要，路线图点位、关联记录和时间线可联动定位当前记录，窄屏地图标签会避让边缘且图例压缩为横向条；390px 视口下模拟关键帧装饰和路线地图元素保持在容器内；检索、CampusVision C1 回退、定位、导出和更新入口使用统一状态提示，初始和切屏状态不会显示空白 toast。
- Electron 桌面壳位于 `desktop/`，GitHub Actions 可构建 Windows 安装包；安装版打开后会优先建立通向 CampusVision C1 的 SSH 隧道，并在软件内连接窗口显示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明；真实检索遇到 CampusVision C1 503 时也会打开同一个内嵌连接窗口并重试；SSH 隧道连接被远端重置时会被主进程吞掉并记录警告，不再弹出 Electron 主进程 JavaScript 错误；桌面端加载 `/demo` 时会清理 renderer cache，并附带 `asset=v0.1.26-ui` 页面参数，避免安装更新后继续显示旧 HTML/CSS/JS；桌面模式右上角提供 `检查更新` 入口，可在应用内下载新版并重启安装。
...
当前 `v0.1.26` 的基线结果：后端测试 `27 passed`，前端脚本、Electron 主进程和 preload 语法检查通过，桌面后端入口编译通过，`npm run test:e2e` 通过 15 个 Chromium 用例，`npm audit --audit-level=low` 无漏洞，`git diff --check` 通过；浏览器 E2E 覆盖 `1280x720`、`680x640` 和 `390x720` 视口下的本地模拟检索、CampusVision C1 单人自动检索、多人查询图框选、低置信查询框过滤、查询图检测失败拦截、无匹配结果返回上传页、检索结果、桌面左侧记录列表、路线图、路线点定位、时间线定位、返回结果页、重新上传流程，以及模拟 CampusVision C1 媒体结果的关键帧预览弹窗、目标人脸框和相似度标注，并检查无页面级横向溢出、无空白 toast、浏览器 console warning/error 为 0；GitHub Actions CI 工作流覆盖 PR 基础检查，发布工作流会生成 Windows 安装包和 Electron 自动更新元数据。
...
2. 打开 `/demo`。
3. 上传目标人脸图片；单人图会自动检索，多人图需要在原图上选择目标人脸。
4. 必要时点击 `开始检索`。
...
GKGuard is the GKGuard C2 workbench for the campus security AI search project: it owns the desktop UI, local backend, result presentation, route visualization, mock fallback, audit features, and external-service proxy layer. CampusVision C1 is the separate video-search service for video upload, frame sampling, face embeddings, person indexing, image-based person search, keyframes, and trajectory output. As of `v0.1.26`, this repository contains the GKGuard C2 backend, the three-screen face-search frontend, the Electron desktop shell, CampusVision C1 auto-connection with an embedded SSH password prompt and connection progress, query-face detection, single-face auto-search, multi-face target selection on the original upload, low-confidence query-box filtering, selected-face search, query-face detection failure blocking, idle upload recovery for no-match results, target-face boxes plus similarity on result keyframes, responsive wide-window layout, full-display media handling for uploaded targets, CampusVision C1 face-crop thumbnails, CampusVision C1 keyframes, a keyframe preview dialog, a left-side desktop result record list, route-point/timeline selection sync, empty states and keyboard focus states, mobile upload/action density improvements, mobile horizontal-scroll hints for result and route records, a mobile current-trajectory summary, narrow-screen route-map label edge avoidance with a compact legend, unified status feedback that does not show empty toasts, Electron renderer cache refresh, stale-backend asset reuse protection, Playwright browser E2E regression, a GKGuard Roadmap field helper script, polished linear UI icons, brand icon assets, an in-app update flow that downloads and restarts to install updates, and the imported CampusVision C1 service source as a separate module.
...
- The GKGuard C2 frontend lives in `backend/app/static/` and currently focuses on three screens: face search, face-search results, and person route map; the layout expands with the desktop window and avoids horizontal overflow down to `680x640`; the upload screen detects query-image faces, auto-searches when there is one face, and overlays face boxes plus detection confidence on the original image when there are multiple faces, hiding boxes below `0.65` when higher-confidence candidates exist so the user can search only the selected target; if query-face detection fails, the UI stays on the upload screen instead of showing false successful results; if CampusVision C1 returns no matched records, the UI returns to an actionable upload state with a Chinese warning instead of staying stuck in loading; the result portrait prefers the selected query-face crop, the desktop result record list is restored to the left side and prefers CampusVision C1 face crops, detail panels render complete CampusVision C1 keyframes with a target-face box and similarity positioned against the rendered image content instead of the surrounding letterbox, and the keyframe preview dialog keeps the same overlay; result and route screens provide a return action to upload a new target; the mobile upload form is compact, action groups use two columns with about 42px touch height, mobile result and route record lists include horizontal-scroll hints and a right fade, the route screen shows route points, start, end, duration, and current-trajectory summary before the map on mobile, narrow-screen route-map labels stay inside the viewport with a compact legend, simulated keyframe decoration and route-map elements stay inside their containers at 390px, and the unified status feedback layer does not show empty toasts during initialization or screen switches.
- The Electron shell lives in `desktop/`; GitHub Actions can build the Windows installer. The packaged app prioritizes the SSH tunnel to CampusVision C1 and shows an embedded connection window with the server account, tunnel target, four-step progress, retry-on-failure state, and password-safety note when needed. It also opens the same embedded connection window and retries when real CampusVision C1 search returns 503, swallows remote tunnel reset errors as warnings instead of showing a main-process JavaScript error dialog, clears the renderer cache before loading `/demo`, appends `asset=v0.1.26-ui` to the page URL so installed updates do not reuse stale HTML/CSS/JS, and exposes a top-right `检查更新` entry in desktop mode for in-app update download and restart-to-install.
...
Current `v0.1.26` baseline: backend tests pass with `27 passed`, frontend script, Electron main-process, and preload syntax checks pass, the desktop backend entrypoint compiles, `npm run test:e2e` passes 15 Chromium cases, `npm audit --audit-level=low` reports no vulnerabilities, and `git diff --check` passes. Browser E2E covers mock search, CampusVision C1 single-face auto-search, multi-face query selection, low-confidence query-box filtering, query-face detection failure blocking, no-match upload recovery, results, the left-side desktop record list, route map, route-point locate, timeline locate, return to results, re-upload, a mocked CampusVision C1 media keyframe preview dialog, target-face overlays, and similarity labels at `1280x720`, `680x640`, and `390x720`, with no page-level horizontal overflow, empty toast, or browser console warning/error. The GitHub Actions CI workflow covers PR checks, and the Release workflow produces the Windows installer and Electron auto-update metadata.
...
2. Open `/demo`.
3. Upload a target face image; single-face images search automatically, while multi-face images require selecting the target face on the original upload.
