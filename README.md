...
当前 `v0.1.27` 的基线结果：后端测试 `27 passed`，前端脚本、Electron 主进程和 preload 语法检查通过，桌面后端入口编译通过，`npm run test:e2e` 通过 18 个 Chromium 用例，CampusVision C1 查询图预处理测试在缺少 OpenCV 的本地环境中按测试级跳过，`npm audit --audit-level=low` 无漏洞，`git diff --check` 通过；浏览器 E2E 覆盖 `1280x720`、`680x640` 和 `390x720` 视口下的本地模拟检索、CampusVision C1 单人自动检索、多人查询图放大弹窗选人、低置信查询候选可见标注、查询图检测失败拦截、无匹配结果返回上传页、检索超时恢复、检索结果、桌面左侧记录列表、路线图、路线点定位、时间线定位、返回结果页、重新上传流程，以及模拟 CampusVision C1 媒体结果的关键帧预览弹窗、目标人脸框和框外相似度标注，并检查无页面级横向溢出、无空白 toast、浏览器 console warning/error 为 0；GitHub Actions CI 工作流覆盖 PR 基础检查，发布工作流会生成 Windows 安装包和 Electron 自动更新元数据。
...
- The Electron shell lives in `desktop/`; GitHub Actions can build the Windows installer. The packaged app prioritizes the SSH tunnel to CampusVision C1 and shows an embedded connection window with the server account, tunnel target, four-step progress, retry-on-failure state, and password-safety note when needed. It also opens the same embedded connection window and retries when real CampusVision C1 search returns 503, swallows remote tunnel reset errors as warnings instead of showing a main-process JavaScript error dialog, clears the renderer cache before loading `/demo`, appends `asset=v0.1.27-ui` to the page URL so installed updates do not reuse stale HTML/CSS/JS, and exposes a top-right `检查更新` entry in desktop mode for in-app update download and restart-to-install.
