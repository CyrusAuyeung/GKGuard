...
当前建议版本：`v0.1.37`。反馈问题时请注明使用的版本标签、安装文件来源、操作系统和 CampusVision C1 服务连接方式。

## 常见问题入口

- 安装包无法打开、GitHub Release 下载、桌面端启动问题：提交缺陷反馈。
- CampusVision C1 服务连接、SSH 隧道、校园网/VPN 可达性问题：提交缺陷反馈，并附 `/c1/status` 脱敏结果。
- 文档错误、接口说明不清楚：提交文档更新。
...
- 离线本地模拟回退属于 GKGuard C2 工作台演示能力。
- 真实 CampusVision C1 检索依赖服务器、网络和 CampusVision C1 数据索引状态。
- CampusCar/UE 当前只保留占位接口规范，不代表已经接入真实车辆或 UE 运行时。
...
Recommended current version: `v0.1.37`. When filing an issue, include the release tag, package source, operating system, and CampusVision C1 service connection path.
...
- CampusCar/UE is currently a placeholder interface specification and does not mean real vehicle or UE runtime integration is complete.
