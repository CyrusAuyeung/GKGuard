...
- C1 连接方式：直连 `10.4.167.122:8000`、本机隧道 `127.0.0.1:18000`，或自定义配置。
- 复现步骤、截图、日志或接口响应。
- 确认已经移除密码、token、真实人脸、真实视频和个人信息。

## 演示支持边界

- 离线 mock fallback 属于 C2 壳体演示能力。
- 真实 C1 检索依赖服务器、网络和 C1 数据索引状态。
...
- C1 connection path: direct `10.4.167.122:8000`, local tunnel `127.0.0.1:18000`, or custom config.
...
- Offline mock fallback is part of the C2 shell demo capability.
- Real C1 search depends on the server, network, and C1 data indexing state.
