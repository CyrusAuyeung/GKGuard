...
仓库已经配置了 Issue 模板和 Pull Request 模板。协作者不需要从零设计格式，按模板填写即可。
...
8. 创建 PR 时必须保留并填写 .github/PULL_REQUEST_TEMPLATE.md，不要删除模板结构；Issue 应使用仓库已有 Issue 模板，架构与集成类事项也应选择仓库已有 Issue 模板中的 `Architecture and integration discussion / 架构与集成讨论`。
...
The repository provides Issue templates and a Pull Request template. Contributors should use the templates instead of inventing their own format.
...
8. When opening a PR, keep and fill in .github/PULL_REQUEST_TEMPLATE.md. Do not delete the template structure. Issues should use the repository Issue templates, and architecture/integration topics should also use the repository Issue template named `Architecture and integration discussion / 架构与集成讨论`.
