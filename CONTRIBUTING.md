...
仓库已经配置了 Issue、Discussion 和 Pull Request 模板。协作者不需要从零设计格式，按模板填写即可。
...
7. 创建 PR 时必须保留并填写 .github/PULL_REQUEST_TEMPLATE.md，不要删除模板结构；Issue 和 Discussion 也应使用仓库已有模板。
...
The repository already provides Issue, Discussion, and Pull Request templates. Contributors should use the templates instead of inventing their own format.
...
7. When opening a PR, keep and fill in .github/PULL_REQUEST_TEMPLATE.md. Do not delete the template structure. Issues and Discussions should also use the repository templates.
