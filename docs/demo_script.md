...
1. 下载并安装 `GKGuard-Setup-0.1.11.exe`。
2. 打开 GKGuard。
...
- 左侧人物照片完整显示上传图或 C1 代表人脸，不应裁掉额头、下巴或主体人脸。
- 检索记录列表展示 C1 关键帧缩略图，而不是默认人物占位图。
- 记录列表显示 C1 摄像头和相似度。
...
Recommended packaged-app flow for `v0.1.11`:

1. Download and install `GKGuard-Setup-0.1.11.exe`.
2. Open GKGuard.
...
- The left target portrait shows the uploaded image or C1 representative face without cutting off the main face.
- The search record list shows C1 keyframe thumbnails instead of the default person placeholder.
