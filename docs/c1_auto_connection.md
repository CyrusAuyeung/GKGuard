...
探测时会访问每个候选 C1 的 `/openapi.json` 和 `/health`。第一个健康检查通过的地址会被选中，后续上传照片时会直接转发到这个 C1。真实检索如果遇到 C1 502/503/504，适配器会继续尝试下一个候选地址；桌面 UI 也会打开 SSH 登录窗口并重试一次。
...
当前桌面端默认支持这个模式：应用启动时会优先检查本机 SSH 隧道；如果尚未通过隧道连接 C1，会弹出提示，选择后会打开一个 PowerShell SSH 窗口。你在该窗口输入服务器密码，GKGuard 不会保存或记录密码。若进入页面后真实检索返回 C1 503，页面也会触发同一个连接窗口并自动重试一次。
...
3. 桌面 UI 会打开 C1 连接提示；用户输入 SSH 密码后自动重试一次。
...
The desktop app supports this mode by default: at startup it checks the local SSH tunnel first. If the tunnel is not connected, the app shows a prompt. If confirmed, it opens a PowerShell SSH window. You type the server password in that window; GKGuard does not store or log the password. If real search returns C1 503 after the page opens, the UI triggers the same connection window and retries once.
...
3. The desktop UI opens the C1 connection prompt; after the user enters the SSH password, it retries once automatically.
