...
安装版进入演示页前会清理 Electron renderer cache，并加载带 `asset=v0.1.28-ui` 参数的 `/demo` 页面。这样安装更新后，桌面端不会继续复用旧的 HTML/CSS/JS 造成布局或功能看起来没有变化。
...
Before entering the demo page, the packaged app clears the Electron renderer cache and loads `/demo` with `asset=v0.1.28-ui`. This prevents installed updates from reusing stale HTML/CSS/JS and making the UI appear unchanged after an upgrade.
