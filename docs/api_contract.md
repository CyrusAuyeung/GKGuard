...
安装版 `v0.3.2` 默认只内置本机隧道候选地址 `http://127.0.0.1:18000`；校园网直连地址必须通过 `C1_BASE_URL`、`C1_CANDIDATE_URLS` 或 `%APPDATA%\GKGuard\c1-connection.json` 显式配置。GKGuard C2 会自动探测候选 CampusVision C1 服务；若未通过隧道连接，桌面端会在软件内连接窗口展示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明。主进程会在发送服务器密码前自动比对受信 SSH 主机密钥；服务器地址或主机密钥变化时必须通过 `sshTunnel.hostFingerprint` 或 `C1_SSH_HOST_FINGERPRINT` 显式更新受信主机信息。受信主机信息缺失或不匹配时，GKGuard C2 会阻断连接而不会弹出人工确认继续连接入口。

`v0.3.2` 保留既有查询图人脸检测、选中人脸以图搜人、结果关键帧标注、记录切换预加载、媒体缓存隔离和路线联动能力，并继续提供 `v0.2.0` 引入的 CampusVision C1 事件智能接口：人物事件、事件观测、查询图候选、人物特征查询和扩展媒体代理。人物特征查询可按上装颜色、眼镜状态、外观倾向、摄像头和可选时间范围筛选 CampusVision C1 事件；GKGuard C2 前端默认显示用户当天 `00:00` 至 `23:59`，但只有用户编辑时间字段后才提交 `time_range`。GKGuard C2 会把 CampusVision C1 返回的事件、观测、人体裁剪图、人脸裁剪图、关键帧、属性命中、人物身份和部分匹配原因归一化为结果页记录。`v0.3.2` 不改变既有 CampusVision C1 代理接口契约，主要修复人物特征搜索中记录头像、关键帧目标框和候选人物按身份聚合筛选的一致性问题。
...
- `person_id` / `personId`：CampusVision C1 人物身份标识；GKGuard C2 用于把同一人物在多个事件中的记录聚合为一个候选人物，并在选择候选人物后筛选左侧检索记录。
...
- `routePoints[]`：路线图轨迹点；人物特征搜索会保留 `recordIndex` 和可选 `eventId`，确保路线点按时间排序后仍能定位到原始结果记录。若 CampusVision C1 旧响应或兼容响应未提供 `recordIndex` / `eventId`，GKGuard C2 仅按路线点下标维护高亮状态，不把超出结果记录数量的路线点强行合成为最后一条记录的稳定映射。
...
返回本地 Top-K 模拟图片检索结果。真实 CampusVision C1 人脸以图搜人请使用 `/c1/search/person-by-image`。
...
- `person_id` / `personId`: CampusVision C1 person identity. GKGuard C2 uses it to group the same person across multiple events into one candidate and to filter the left result records after a candidate is selected.
