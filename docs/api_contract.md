...
安装版 `v0.1.25` 还内置候选地址 `http://127.0.0.1:18000` 和 `http://10.4.167.122:8000`，优先使用本机 SSH 隧道。GKGuard C2 会自动探测候选 CampusVision C1 服务；若未通过隧道连接，桌面端会在软件内连接窗口展示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明，并用本次输入的服务器密码建立 SSH 隧道。真实检索遇到 CampusVision C1 502/503/504 时，适配器会尝试下一个候选地址。本版修正查询图人脸检测与选中人脸检索：上传图只有一张人脸时自动检索，多张人脸时必须在原图上选择目标人脸，检测失败时停留在上传页而不展示模拟命中；结果页人物照片显示选中人脸裁切图，桌面端记录列表恢复在左侧，结果关键帧和预览弹窗会框出目标人脸并显示相似度。其余响应式布局、关键帧预览、路线联动、统一状态提示、应用内更新和旧后台资源复用保护继续保留。
...
- `faceCount>1`：在上传原图上显示所有人脸框和检测置信度，等待用户选择目标人脸。
...
如果 CampusVision C1 服务不可用，GKGuard C2 返回结构化错误；当前桌面 UI 会先触发 CampusVision C1 连接窗口并重试一次。未上传图片时仍可进入本地模拟流程；已上传图片时，查询图人脸检测或真实检索失败会停留在上传页并提示重试，不展示模拟命中结果。
...
The packaged `v0.1.25` app also has built-in candidates `http://127.0.0.1:18000` and `http://10.4.167.122:8000`, preferring the local SSH tunnel. GKGuard C2 probes candidate CampusVision C1 URLs automatically; if the tunnel is not connected, the desktop app prompts for the server password inside the app, shows connection progress, and creates the SSH tunnel. When real search hits CampusVision C1 502/503/504, the adapter tries the next candidate URL. This version fixes query-face detection and selected-face search: single-face uploads auto-search, multi-face uploads must select the target face on the original image, detection failures stay on the upload screen instead of showing mock hits, the result portrait shows the selected face crop, the desktop record list is restored to the left side, and result keyframes plus the preview dialog overlay the target face with similarity. The responsive layout, keyframe preview, route selection sync, unified status feedback, in-app updates, and stale-backend asset protection remain in place.
...
- `faceCount>1`: overlay all face boxes and detection confidence on the original upload, then wait for the user to select the target face.
