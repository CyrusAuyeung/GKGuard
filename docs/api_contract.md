...
- `X-GKGuard-Audit-Token`：必须匹配后端环境变量 `GKGUARD_AUDIT_TOKEN`。未配置或不匹配时返回 `403 AUDIT_FORBIDDEN`。

Query 参数：

- `limit`：默认 `20`，最大 `100`。

返回敏感演示操作产生的审计日志。后端读取时只保留最近 `limit` 行到内存，并在日志超过 1 MiB 时压缩保留最近 2000 行。
...
- `X-GKGuard-Audit-Token`: must match the backend `GKGUARD_AUDIT_TOKEN` environment variable. Missing, unset, or mismatched tokens return `403 AUDIT_FORBIDDEN`.

Query parameters:

- `limit`: default `20`, max `100`.

Returns audit logs generated by sensitive demo actions. The backend keeps only the latest `limit` lines in memory while reading, and compacts the log to the latest 2000 lines after it exceeds 1 MiB.
