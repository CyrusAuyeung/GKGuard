...
如果 C1 不可用，C2 返回结构化错误；当前视觉 UI 会捕获错误并回退本地模拟数据。
...
- `records[]`: keyframe records for the result screen.
...
If C1 is unavailable, C2 returns a structured error. The current visual UI catches that error and falls back to local mock data.
