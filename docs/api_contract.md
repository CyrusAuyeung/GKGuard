...
安装版 `v0.1.13` 还内置候选地址 `http://127.0.0.1:18000` 和 `http://10.4.167.122:8000`，优先使用本机 SSH 隧道。C2 会自动探测候选 C1；若未通过隧道连接，桌面端会提示打开 PowerShell SSH 窗口并由用户输入服务器密码。真实检索遇到 C1 502/503/504 时，适配器会尝试下一个候选地址。
...
The packaged `v0.1.13` app also has built-in candidates `http://127.0.0.1:18000` and `http://10.4.167.122:8000`, preferring the local SSH tunnel. C2 probes candidate C1 URLs automatically; if the tunnel is not connected, the desktop app prompts to open a PowerShell SSH window where the user enters the server password. When real search hits C1 502/503/504, the adapter tries the next candidate URL.
