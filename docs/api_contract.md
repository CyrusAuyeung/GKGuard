...
GKGuard C2 会对成功读取的 CampusVision C1 媒体字节使用进程内短期缓存，缓存键包含当前 CampusVision C1 base URL，默认缓存 300 秒、最多 64 项，可通过 `C1_MEDIA_CACHE_TTL_SEC` 和 `C1_MEDIA_CACHE_MAX_ITEMS` 调整。常规代理请求还会复用短期 CampusVision C1 健康状态探测结果，默认 15 秒，可通过 `C1_STATUS_CACHE_TTL_SEC` 调整；`GET /c1/status` 仍执行实时探测，探测失败或身份校验失败会清理对应地址的旧健康缓存。
...
GKGuard C2 uses a short-lived in-process cache for successfully fetched CampusVision C1 media bytes, with the current CampusVision C1 base URL included in the cache key, defaulting to 300 seconds and 64 items. Tune it with `C1_MEDIA_CACHE_TTL_SEC` and `C1_MEDIA_CACHE_MAX_ITEMS`. Normal proxy requests also reuse short-lived healthy CampusVision C1 status probes, defaulting to 15 seconds through `C1_STATUS_CACHE_TTL_SEC`; `GET /c1/status` still performs a fresh probe and clears stale healthy cache entries when a probe or identity check fails.
