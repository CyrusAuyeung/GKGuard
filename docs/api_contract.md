...
安装版 `v0.1.26` 还内置候选地址 `http://127.0.0.1:18000` 和 `http://10.4.167.122:8000`，优先使用本机 SSH 隧道。GKGuard C2 会自动探测候选 CampusVision C1 服务；若未通过隧道连接，桌面端会在软件内连接窗口展示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明，并用本次输入的服务器密码建立 SSH 隧道。真实检索遇到 CampusVision C1 502/503/504 时，适配器会尝试下一个候选地址。本版继续收紧查询图人脸检测与选中人脸检索：上传图只有一张有效候选人脸时自动检索，多张有效候选人脸时必须在原图上选择目标人脸，存在高置信候选时会隐藏低于 `0.65` 的低置信检测框，检测失败或无匹配结果时停留在上传页而不展示模拟命中；结果页人物照片显示选中人脸裁切图，桌面端记录列表保持在左侧，结果关键帧和预览弹窗会按实际图片内容区域框出目标人脸并显示相似度，避免目标框落在左上角或受黑边偏移。Electron 桌面端会在加载页面前清理 renderer cache，并通过 `asset=v0.1.26-ui` 页面参数避免安装更新后继续显示旧页面。其余响应式布局、关键帧预览、路线联动、统一状态提示、应用内更新和旧后台资源复用保护继续保留。
...
- `faceCount=1`：自动选择唯一人脸并直接检索。
- `faceCount>1`：在上传原图上显示有效候选人脸框和检测置信度，等待用户选择目标人脸；当存在高置信候选时，前端会隐藏低于 `0.65` 的低置信检测框，减少误检框堆叠。
...
如果 CampusVision C1 服务不可用，GKGuard C2 返回结构化错误；当前桌面 UI 会先触发 CampusVision C1 连接窗口并重试一次。未上传图片时仍可进入本地模拟流程；已上传图片时，查询图人脸检测失败、真实检索失败或 `records[]` 为空都会停留在上传页并提示重试，不展示模拟命中结果，也不保持“检索中”状态。
...
The packaged `v0.1.26` app also has built-in candidates `http://127.0.0.1:18000` and `http://10.4.167.122:8000`, preferring the local SSH tunnel. GKGuard C2 probes candidate CampusVision C1 URLs automatically; if the tunnel is not connected, the desktop app prompts for the server password inside the app, shows connection progress, and creates the SSH tunnel. When real search hits CampusVision C1 502/503/504, the adapter tries the next candidate URL. This version further tightens query-face detection and selected-face search: uploads with one effective candidate face auto-search, uploads with multiple effective candidates require selecting the target face on the original image, boxes below `0.65` are hidden when higher-confidence candidates exist, and detection failures or no-match results stay on the upload screen instead of showing mock hits. The result portrait shows the selected face crop, the desktop record list stays on the left side, and result keyframes plus the preview dialog position the target overlay against the rendered image content so boxes do not fall back to the top-left corner or drift because of letterboxing. Electron clears the renderer cache before loading the page and appends `asset=v0.1.26-ui` so installed updates do not reuse stale HTML/CSS/JS. The responsive layout, keyframe preview, route selection sync, unified status feedback, in-app updates, and stale-backend asset protection remain in place.
...
- `faceCount=1`: auto-select the only face and search directly.
- `faceCount>1`: overlay effective candidate face boxes and detection confidence on the original upload, then wait for the user to select the target face; when higher-confidence candidates exist, the frontend hides boxes below `0.65` to reduce false-positive overlap.
...
If the CampusVision C1 service is unavailable, GKGuard C2 returns a structured error. The current desktop UI first opens the CampusVision C1 connection prompt and retries once. Local mock data is still available when no image has been uploaded; after an image upload, query-face detection failures, real-search failures, or an empty `records[]` result keep the UI on the upload screen with a retry/error message instead of falling back to mock results or staying in a loading state.
