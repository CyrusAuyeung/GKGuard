...
安装版 `v0.1.24` 还内置候选地址 `http://127.0.0.1:18000` 和 `http://10.4.167.122:8000`，优先使用本机 SSH 隧道。GKGuard C2 会自动探测候选 CampusVision C1 服务；若未通过隧道连接，桌面端会在软件内连接窗口展示服务器账号、隧道目标、四步连接进度、失败重试和密码安全说明，并用本次输入的服务器密码建立 SSH 隧道。真实检索遇到 CampusVision C1 502/503/504 时，适配器会尝试下一个候选地址。本版新增查询图人脸检测与选中人脸检索：上传图只有一张人脸时自动检索，多张人脸时在原图上显示检测框和检测置信度，并把用户选择的人脸通过 `query_face_index` 传给 CampusVision C1；结果关键帧和预览弹窗会框出目标人脸并显示相似度。其余响应式布局、关键帧预览、路线联动、统一状态提示、应用内更新和旧后台资源复用保护继续保留。
...
- `records[].faceBox`：命中关键帧中的目标人脸框，GKGuard C2 前端用它在详情关键帧和预览弹窗上标注目标人脸，并显示 `records[].similarity`。
- `routePoints[]`：路线图轨迹点。
...
The packaged `v0.1.24` app also has built-in candidates `http://127.0.0.1:18000` and `http://10.4.167.122:8000`, preferring the local SSH tunnel. GKGuard C2 probes candidate CampusVision C1 URLs automatically; if the tunnel is not connected, the desktop app prompts for the server password inside the app, shows connection progress, and creates the SSH tunnel. When real search hits CampusVision C1 502/503/504, the adapter tries the next candidate URL. This version adds query-face detection and selected-face search: when the uploaded image has one face, the UI auto-selects it and searches directly; when it has multiple faces, the UI overlays face boxes and detection confidence on the original image and forwards the selected target as `query_face_index` to CampusVision C1. Result keyframes and the preview dialog overlay the matched target face with the similarity score. The responsive layout, keyframe preview, route selection sync, unified status feedback, in-app updates, and stale-backend asset protection remain in place.
...
- `records[].faceBox`: target-face box in the matched keyframe; GKGuard C2 uses it to overlay the target face and `records[].similarity` on detail keyframes and preview dialogs.
