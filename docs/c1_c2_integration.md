...
本文记录 CampusVision C1 服务与 GKGuard C2 工作台的职责边界、运行连接方式、已实现代理接口、字段映射和联调检查。CampusVision C1 是视频检索服务，负责视频索引、人脸向量、人物库、以图搜人、人物事件、事件观测、外观属性分析、关键帧和轨迹输出；GKGuard C2 是桌面工作台和本地代理层，负责 UI、CampusVision C1 连接、结果归一化、路线展示、本地模拟回退和 CampusCar/UE 占位接口规范。当前 `v0.3.1` 已包含 GKGuard C2 到 CampusVision C1 的真实以图搜人链路、人物特征事件搜索链路、查询图人脸检测预处理与重试、多人放大选人、事件/观测媒体代理、结果页属性摘要、结果关键帧目标框和框外相似度标注、结果记录切换关键帧预加载与无黑屏切换、候选人物抽屉、事件详情抽屉和响应式工作台布局。
...
当前版本还要求媒体请求发起前和返回前重新校验连接代次；如果 CampusVision C1 候选地址在健康探测、GET 发出前或 GET 返回前变化，GKGuard C2 会重新解析候选或返回可重试错误。中等宽度结果页会固定人物照片列的响应式范围，避免人物照片遮挡数据来源和命中记录信息；桌面结果页保持目标人物与检索记录在左侧，证据详情在右侧，并通过抽屉承载候选人物和事件详情。
...
- 人物特征搜索联调时，应确认 CampusVision C1 返回事件、属性分数、未命中条件和可访问媒体 URL，GKGuard C2 结果页能展示事件记录、属性摘要、关键帧、缩略图和路线点。
...
The current version also re-checks the connection generation before sending media requests and before returning media responses. If the CampusVision C1 candidate changes during healthy-candidate probing, before the GET is sent, or before the GET response is returned, GKGuard C2 re-resolves candidates or returns a retryable error. Medium-width result layouts constrain the target-portrait column so it cannot cover the source and hit-count summary; desktop result pages keep the target person and record list on the left, evidence detail on the right, and use drawers for candidate people and event detail.
