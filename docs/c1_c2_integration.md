...
本文记录 CampusVision C1 服务与 GKGuard C2 工作台的职责边界、运行连接方式、已实现代理接口、字段映射和联调检查。CampusVision C1 是视频检索服务，负责视频索引、人脸向量、人物库、以图搜人、人物事件、事件观测、外观属性分析、关键帧和轨迹输出；GKGuard C2 是桌面工作台和本地代理层，负责 UI、CampusVision C1 连接、结果归一化、路线展示、本地模拟回退和 CampusCar/UE 占位接口规范。当前 `v0.3.2` 已包含 GKGuard C2 到 CampusVision C1 的真实以图搜人链路、人物特征事件搜索链路、查询图人脸检测预处理与重试、多人放大选人、事件/观测媒体代理、结果页属性摘要、结果关键帧目标框和框外相似度标注、结果记录切换关键帧预加载与无黑屏切换、候选人物按身份聚合筛选和响应式工作台布局。

GKGuard C2 代理和 CampusVision C1 原生查询图候选接口均使用 Query 参数传递检索控制项，文件继续使用 multipart 字段；CampusVision C1 4xx 输入或校验错误会保留错误详情，不再被误判为需要桌面端重新连接；CampusVision C1 5xx 服务端错误会被脱敏为结构化服务不可用状态，不向前端透传内部详情。CampusVision C1 重建索引提交失败时会恢复既有索引、旧帧目录、人物索引和相关 appearance sessions；写入路径使用数据库写锁隔离，读取接口不因重建索引全局串行化。人物特征搜索的路线点按事件时间排序，并通过 `recordIndex` / `eventId` 映射回原始结果记录；人物特征搜索页默认显示用户当天 `00:00` 至 `23:59`，但只有用户编辑时间字段后才把 `time_range` 提交到代理层，避免默认显示值隐式过滤历史事件。如果默认最高分记录不在可绘制路线点内，GKGuard C2 前端会默认选中第一个路线点对应记录，避免路线页无高亮或路线图、时间线点击后选中错误记录。若兼容响应中的路线点未提供 `recordIndex` / `eventId`，GKGuard C2 前端只按路线点下标维护当前高亮，不把超出记录数量的路线点归并为最后一条记录的稳定映射。GKGuard C2 前端只访问 GKGuard C2 后端，GKGuard C2 后端再通过 `/c1/...` 代理访问 CampusVision C1。

当前版本还要求媒体请求发起前和返回前重新校验连接代次；如果 CampusVision C1 候选地址在健康探测、GET 发出前或 GET 返回前变化，GKGuard C2 会重新解析候选或返回可重试错误。中等宽度结果页会固定人物照片列的响应式范围，避免人物照片遮挡数据来源和命中记录信息；桌面结果页保持目标人物与检索记录在左侧，证据详情在右侧，并通过候选人物抽屉按人物身份复核和筛选记录。
...
| `person_id` / `personId` | 人物身份标识；人物特征搜索中用于把同一人物的多条事件记录聚合为一个候选人物，并在选中候选后筛选左侧记录。 |
...
| `events[]` | 人物特征搜索或人物事件列表，GKGuard C2 映射为结果页 `records[]` 和路线页 `routePoints[]`。 |
...
| `attributes` / `condition_scores` / `failed_conditions` | 人物特征搜索命中条件、分项分数和未满足条件，GKGuard C2 在结果页相关信息区域展示。 |
...
- CampusVision C1 会限制查询图数量、查询图上传体积、视频上传体积、解码后图片尺寸和视频索引帧数；不可解码查询图和超限请求返回结构化错误并清理已经保存的临时上传。
- 前端调用 `/c1/search/person-by-image` 时会在需要时附带 `query_face_index`，CampusVision C1 只用选中的查询图人脸 embedding 检索。
- 人物特征搜索不需要上传图片；GKGuard C2 前端将筛选条件提交到 `/c1/query/person-attributes`，后端转发给 CampusVision C1 并把事件结果归一化为结果页记录。低分或部分匹配结果保留分数和未命中条件供人工复核，不表述为确定身份。
- GKGuard C2 可以通过 `/c1/events`、`/c1/persons/{person_id}/events` 和 `/c1/events/{event_id}/observations` 读取 CampusVision C1 事件与观测明细，用于轨迹和证据展示；当前结果页的事件证据以主详情区为准，不再提供重复的事件详情按钮。
...
- 人物特征搜索联调时，应确认 CampusVision C1 返回事件、人物身份、属性分数、未命中条件、人脸框和可访问媒体 URL，GKGuard C2 结果页能展示事件记录、属性摘要、关键帧、缩略图、目标框和路线点；候选人物抽屉应按人物身份去重，并在选中候选后只展示该人物相关记录。
...
This document records the responsibility boundary, runtime connection, implemented proxy endpoints, field mapping, and integration checklist between the CampusVision C1 service and the GKGuard C2 workbench. CampusVision C1 is the video-search service for video indexing, face embeddings, person indexing, image search, person events, event observations, appearance-attribute analysis, keyframes, and trajectory output. GKGuard C2 is the desktop workbench and local proxy layer for UI, CampusVision C1 connectivity, result normalization, route display, mock fallback, and CampusCar/UE placeholder interface specifications. As of `v0.3.2`, the real GKGuard C2-to-CampusVision C1 integration includes image-based person search, person-attribute event search, query-face preprocessing and retry, enlarged multi-face selection, event/observation media proxying, result-page attribute summaries, target-face overlays with outside-box similarity labels, preloaded no-black-screen keyframe switching, identity-grouped candidate filtering, and a responsive workbench layout.

The GKGuard C2 proxy and native CampusVision C1 query-image candidate endpoint both pass search controls as Query parameters while keeping the uploaded file as multipart data; CampusVision C1 4xx input or validation errors preserve their details and are not treated as desktop reconnection failures, while CampusVision C1 5xx service errors are sanitized into structured service-unavailable states without forwarding internal details to the frontend. If a CampusVision C1 re-index commit fails, the service restores the previous index, frame directory, person-index state, and related appearance sessions; write paths use a database write lock, while read APIs are not globally serialized by the re-index lock. Person-attribute route points are sorted by event time and map back to original result records through `recordIndex` / `eventId`; the person-attribute search page displays the user's current day from `00:00` to `23:59` by default, but submits `time_range` to the proxy only after the user edits a time field, so the displayed defaults do not implicitly filter historical events. If the default top-score record is not represented by a plotted route point, the GKGuard C2 frontend selects the first route-point record by default, preventing route screens with no active point and wrong record selection after route-map or timeline clicks. If compatibility route points omit `recordIndex` / `eventId`, the GKGuard C2 frontend keeps the current highlight by route-point index and does not collapse out-of-range route points into a stable mapping to the last record. The GKGuard C2 frontend talks only to the GKGuard C2 backend, and the GKGuard C2 backend accesses CampusVision C1 through `/c1/...` proxy endpoints.

The current version also re-checks the connection generation before sending media requests and before returning media responses. If the CampusVision C1 candidate changes during healthy-candidate probing, before the GET is sent, or before the GET response is returned, GKGuard C2 re-resolves candidates or returns a retryable error. Medium-width result layouts constrain the target-portrait column so it cannot cover the source and hit-count summary; desktop result pages keep the target person and record list on the left, evidence detail on the right, and use the candidate-person drawer for identity review and record filtering.
...
| `person_id` / `personId` | Person identity. Person-attribute search uses it to group multiple event records for the same person into one candidate and to filter the left record list after a candidate is selected. |
...
- CampusVision C1 limits query-image count, query-image upload size, video upload size, decoded image dimensions, and video indexing frames; undecodable query images and over-limit requests return structured errors and clean up saved temporary uploads.
...
- GKGuard C2 can read CampusVision C1 event and observation detail through `/c1/events`, `/c1/persons/{person_id}/events`, and `/c1/events/{event_id}/observations` for trajectory and evidence presentation. The current result page treats the main detail area as the event evidence surface and no longer exposes a duplicate event-detail button.
