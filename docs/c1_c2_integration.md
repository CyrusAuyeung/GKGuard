...
- GKGuard C2 对成功读取的 CampusVision C1 媒体响应使用进程内短期缓存，缓存键包含当前 CampusVision C1 base URL，并复用短期 CampusVision C1 健康状态探测结果；实时状态探测失败或身份校验失败时会清理对应地址的旧健康缓存。
...
- GKGuard C2 uses a short-lived in-process cache for successful CampusVision C1 media responses, with the current CampusVision C1 base URL included in the cache key, and reuses short-lived healthy CampusVision C1 status probes; failed fresh probes or identity checks clear stale healthy cache entries for the affected address.
