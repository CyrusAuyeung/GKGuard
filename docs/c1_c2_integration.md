...
GKGuard C2 代理和 CampusVision C1 原生查询图候选接口均使用 Query 参数传递检索控制项，文件继续使用 multipart 字段；CampusVision C1 4xx 输入或校验错误会保留错误详情，不再被误判为需要桌面端重新连接；CampusVision C1 5xx 服务端错误会被脱敏为结构化服务不可用状态，不向前端透传内部详情。CampusVision C1 重建索引提交失败时会恢复既有索引、旧帧目录、人物索引和相关 appearance sessions；写入路径使用数据库写锁隔离，读取接口不因重建索引全局串行化。人物特征搜索的路线点按事件时间排序，并通过 `recordIndex` / `eventId` 映射回原始结果记录；如果默认最高分记录不在可绘制路线点内，GKGuard C2 前端会默认选中第一个路线点对应记录，避免路线页无高亮或路线图、时间线点击后选中错误记录。若兼容响应中的路线点未提供 `recordIndex` / `eventId`，GKGuard C2 前端只按路线点下标维护当前高亮，不把超出记录数量的路线点归并为最后一条记录的稳定映射。GKGuard C2 前端只访问 GKGuard C2 后端，GKGuard C2 后端再通过 `/c1/...` 代理访问 CampusVision C1。
