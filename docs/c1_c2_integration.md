...
本文记录 CampusVision C1 服务与 GKGuard C2 工作台的职责边界、运行连接方式、已实现代理接口、字段映射和联调检查。CampusVision C1 是视频检索服务，负责视频索引、人脸向量、人物库、以图搜人、人物事件、事件观测、外观属性分析、关键帧和轨迹输出；GKGuard C2 是桌面工作台和本地代理层，负责 UI、CampusVision C1 连接、结果归一化、路线展示、本地模拟回退和 CampusCar/UE 占位接口规范。当前 `v0.2.2` 已包含 GKGuard C2 到 CampusVision C1 的真实以图搜人链路、人物特征事件检索链路、查询图人脸检测预处理与重试、多人显式放大选人按钮、事件/观测媒体代理、结果页属性摘要、结果关键帧目标框和框外相似度标注、结果记录切换关键帧预加载与无黑屏切换，以及 CampusVision C1 候选地址允许列表、服务身份检查、受信 SSH 主机密钥自动比对、外部暴露时 API key、受保护媒体/视频列表/人物库/事件接口、按 CampusVision C1 响应地址、API key、候选配置和连接代次隔离的 GKGuard C2 进程内媒体缓存、媒体缓存容量上限、失败状态探测清理旧健康缓存、不可解码查询图 400、查询图超限 413 与临时上传目录清理，以及同一视频重索引前清理旧事件、人物观测、人脸记录和旧帧目录并刷新受影响人物索引、重建相关 appearance sessions。GKGuard C2 代理和 CampusVision C1 原生查询图候选接口均使用 Query 参数传递检索控制项，文件继续使用 multipart 字段；CampusVision C1 4xx 输入或校验错误会保留错误详情，不再被误判为需要桌面端重新连接；CampusVision C1 5xx 服务端错误会被脱敏为结构化服务不可用状态，不向前端透传内部详情。人物特征检索的路线点按事件时间排序，并通过 `recordIndex` / `eventId` 映射回原始结果记录；如果默认最高分记录不在可绘制路线点内，GKGuard C2 前端会默认选中第一个路线点对应记录，避免路线页无高亮或路线图、时间线点击后选中错误记录。若兼容响应中的路线点未提供 `recordIndex` / `eventId`，GKGuard C2 前端只按路线点下标维护当前高亮，不把超出记录数量的路线点归并为最后一条记录的稳定映射。GKGuard C2 前端只访问 GKGuard C2 后端，GKGuard C2 后端再通过 `/c1/...` 代理访问 CampusVision C1。
