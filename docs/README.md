...
本文档用于维护时快速定位材料。当前文档以 `v0.3.2` 为最新状态，历史发布说明保留各版本发布时的语境。
...
- [releases/v0.3.2.md](releases/v0.3.2.md)：当前最新版本说明，记录人物特征搜索记录头像同步、目标框标注、候选人物按身份聚合、候选筛选记录和事件详情入口移除。
- [releases/v0.3.1.md](releases/v0.3.1.md)：GKGuard C2 工作台回归修复、入口文案统一、结果页标题/更新按钮避让、事件详情和候选人物抽屉修复、人物特征搜索目标框、当天时间默认显示、显式时间范围提交和日期输入限制。
...
- [releases/v0.2.0.md](releases/v0.2.0.md)：CampusVision C1 事件与人物特征查询接口、GKGuard C2 事件搜索代理、人物特征搜索 UI、事件关键帧/人体图/人脸图媒体代理、结果页属性摘要，以及 Windows/macOS/Linux 桌面发布资产。
...
Use this page to locate maintenance material quickly. The current-state documentation tracks `v0.3.2`; historical Release notes keep the context from each release.
...
- [releases/v0.3.2.md](releases/v0.3.2.md): latest release notes covering person-attribute record portrait synchronization, target overlays, identity-grouped candidates, candidate-filtered records, and removal of the duplicate event-detail entry.
- [releases/v0.3.1.md](releases/v0.3.1.md): GKGuard C2 workbench regression fixes, unified entry wording, result-heading/update-button spacing, event-detail and candidate-person drawer fixes, target overlays for person-attribute search, current-day time display, explicit time-range submission, and date-input limits.
...
- [releases/v0.2.0.md](releases/v0.2.0.md): CampusVision C1 event and person-attribute query endpoints, GKGuard C2 event-search proxies, the person-attribute search UI, event keyframe/body/face media proxies, result-page attribute summaries, and Windows/macOS/Linux desktop release assets.
