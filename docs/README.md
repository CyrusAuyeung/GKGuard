...
- [releases/v0.1.37.md](releases/v0.1.37.md)：当前最新版本说明，记录结果页记录切换关键帧预加载、失败预加载重试、按 CampusVision C1 响应地址、API key、候选配置和连接代次隔离的 GKGuard C2 进程内媒体缓存、媒体缓存容量上限、媒体请求发起前连接代次重校验、CampusVision C1 健康状态探测复用、中等宽度人物照片布局修复和无黑屏切换体验。
...
- [releases/v0.1.37.md](releases/v0.1.37.md): latest release notes covering preloaded keyframe switching on the result page, failed-preload retries, GKGuard C2 in-process media caching scoped by CampusVision C1 responding address, API key, candidate configuration, and connection generation, media-cache byte limits, connection-generation rechecks before media requests are sent, reused healthy CampusVision C1 status probes, the medium-width target-portrait layout fix, and no-black-screen record switching.
