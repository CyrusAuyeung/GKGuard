...
本文档用于维护时快速定位材料。当前文档以 `v0.3.0` 为最新状态，历史发布说明保留各版本发布时的语境。
...
- [releases/v0.3.0.md](releases/v0.3.0.md)：当前最新版本说明，记录 GKGuard C2 工作台式 UI 重构、固定导航、证据优先详情布局、候选人物抽屉、事件详情抽屉和响应式布局验证。
...
Use this page to locate maintenance material quickly. The current-state documentation tracks `v0.3.0`; historical Release notes keep the context from each release.
...
- [releases/v0.3.0.md](releases/v0.3.0.md): latest release notes covering the GKGuard C2 workbench UI redesign, fixed navigation, evidence-first detail layout, candidate-person drawer, event-detail drawer, and responsive-layout verification.
