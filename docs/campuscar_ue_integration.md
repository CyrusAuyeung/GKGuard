...
- UE/ROS bridge 准备状态占位。
- 后续复核证据使用的 HLS/RTSP/video URL 占位。

GKGuard 不应提供：

- STM32 串口控制。
- 桌面 app 内的 ROS2 节点所有权。
- Unreal Engine 运行时打包。
- 对 `GKD_Station_Qiyi.exe` 的直接依赖。

## 外部信号

控制组材料给出的 UE Bridge 流程包括：

- 命令发布 topic：`/U2RTopic_Command`
- 位姿反馈 topic：`/R2UTopic_Pos`
- 文本/状态反馈 topic：`/R2UTopic_Text`
...
`POST /car-tasks/mock-dispatch` 保持 mock-only。它会回显集成字段，并返回 `bridge_contract`，方便前端和其他小组明确真实服务未来接入点。

`GET /car-tasks/ue-bridge-status` 返回确定性的 mock readiness 对象，用于 UI 和演示。它当前不检查真实 rosbridge 进程。

## 后续替换计划

1. 保留 `/car-tasks/mock-dispatch` 用于离线演示。
2. 控制/UE 团队提供稳定消息 schema 后，再新增真实 adapter endpoint。
3. 在 adapter 中把事件任务创建映射到 `/U2RTopic_Command`。
4. 在 adapter 中订阅 `/R2UTopic_Pos` 和 `/R2UTopic_Text`。
...
`POST /car-tasks/mock-dispatch` stays mock-only. It echoes integration fields and returns a `bridge_contract` block so the frontend and other groups can see exactly where real services will plug in.
...
2. Add a real adapter endpoint only after the control/UE teams provide stable message schemas.
